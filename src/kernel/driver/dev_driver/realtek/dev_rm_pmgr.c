 ...
 * INPUT:    device_id, group_id, dev_rule_id, ace_entry_p, dev_action_p
 * OUTPUT:   NONE.
 * RETURN:   TRUE/FALSE.
    ...
    const DEVRM_ACTION_T *dev_action_p)
{
    ...
    if (dev_action_p != NULL)
    {
        memcpy(&msg_data_p->data.setrule.req.action, dev_action_p, sizeof(DEVRM_ACTION_T));
    }
                             ...
                             DEVRM_ACTION_T *action_p)
{
    ...
    if (action_p != NULL)
    {
        memcpy(&msg_data_p->data.updaterule.req.action, action_p, sizeof(DEVRM_ACTION_T));
    }
...
BOOL_T DEVRM_PMGR_CreateMeter(UI32_T device_id,UI32_T dev_rule_id, DEVRM_POLICER_METER_T *meter_entry_p, BOOL_T need_reinstalled)
{
    ...
    {
        memcpy(&msg_data_p->data.create_meter.req.meter_entry, meter_entry_p, sizeof(DEVRM_POLICER_METER_T));
    }
    ...
    const DEVRM_POLICER_METER_T *meter_entry_p)
{
    ...
    if(meter_entry_p != NULL)
    {
        memcpy(&msg_data_p->data.update_meter.req.meter_entry, meter_entry_p, sizeof(DEVRM_POLICER_METER_T));
    }
                            ...
                            //DEVRM_ActionEntry_T *action_entry)
{
    ...
    memcpy(&msg_data_p->data.setaction.req.action, action_p, sizeof(DEVRM_ACTION_T));
