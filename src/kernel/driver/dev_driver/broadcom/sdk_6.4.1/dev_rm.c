...
#include <customer/accton_rate.h>
...
static DEVRM_VirtualGroup_T * _DEVRM_FindVirtualGroupByRuleId(
    UI32_T device_id,
    UI32_T dev_rule_id);
    ...
    for(device_id = 0; device_id < soc_ndev; device_id++)
    {
       ret_value = bcm_field_init(device_id);
        ...
        if ( (SOC_IS_RAVEN(device_id)) || (SOC_IS_FIREBOLT2(device_id)) || (SOC_IS_FIREBOLT(device_id)) )
        {
            ...
            /*  2015-03-13, Polly Hsu:
             *  the egress ifg setting is moved to DEV_SWDRV_ResetOnDemand with
             *  the ingress ifg setting
             */
        }
...
BOOL_T DEVRM_CreateGroup(UI32_T device_id, DEVRM_GroupEntry_T *group_entry)
{
    ...
    i = device_id;
    {
#if 0 /* TODO: wakka: udf config is not ready and shall be revised for BCM SDK 6 */
        int uid;
...
#endif /* TODO: wakka: udf config is not ready and shall be revised for BCM SDK 6 */
    ...
    const DEVRM_ActionEntry_T *action_entry_p)
{
    ...
    {
        action_iter_p = &action_entry_p[i];
...
BOOL_T DEVRM_UpdateRule(UI32_T device_id, UI32_T dev_rule_id, DEVRM_AceEntry_T *ace_entry, DEVRM_MeterEntry_T *meter_entry, DEVRM_ActionEntry_T *action_entry, UI32_T action_entry_num)
{
    ...
    ret_value = bcm_field_entry_policer_get(device_id, dev_rule_id, 0, &policer_id);
    if (ret_value != BCM_E_NONE)
    {
        policer_id = 0;
    }
    ...
    if (policer_id != 0)
    {
        ret_value = bcm_field_entry_policer_detach(device_id, dev_rule_id, 0);
        if (BCM_E_NONE != ret_value)
        {
            printf("DEVRM_UpdateRule Error! bcm_field_entry_policer_detach[%s]\n", bcm_errmsg(ret_value));
            return FALSE;
        }

        ret_value = bcm_policer_destroy(device_id, policer_id);
        if ((BCM_E_NONE != ret_value) && (BCM_E_BUSY != ret_value))
        {
            printf("DEVRM_UpdateRule Error! bcm_policer_destroy[%s]\n", bcm_errmsg(ret_value));
            return FALSE;
        }
    }
    ...
    if(meter_entry != NULL)
    {
        if (TRUE != DEVRM_CreateMeter(device_id, dev_rule_id, meter_entry, FALSE))
        {
            printf("DEVRM_UpdateRule Error! DEVRM_CreateMeter failed.\n");
            return FALSE;
        }
    }
    //counter ??
...
static int DEVRM_FieldQualify(UI32_T device_id, UI32_T dev_rule_id, DEVRM_AceEntry_T *ace_entry)
{
    ...
    {
        ret_value = bcm_field_qualify_SrcPort(device_id, dev_rule_id, ace_entry->srcmodid_data, ace_entry->srcmodid_mask, 0, 0);
        if (ret_value != BCM_E_NONE)
            printf("DEVRM_FieldQualify Error! bcm_field_qualify_SrcModid[%s]\n", bcm_errmsg(ret_value));
    }

#if 0 /* obsolete by BCM SDK 6 */
    ...
    {
        ret_value = bcm_field_qualify_DstPort(device_id, dev_rule_id, ace_entry->dstmodid_data, ace_entry->dstmodid_mask, 0, 0);
        if (ret_value != BCM_E_NONE)
    ...
    {
        UI32_T ip_addr = L_STDLIB_Ntoh32(ace_entry->srcip_data);
        UI32_T ip_mask = L_STDLIB_Ntoh32(ace_entry->srcip_mask);

        ret_value = bcm_field_qualify_SrcIp(device_id, dev_rule_id, ip_addr, ip_mask);
        if (ret_value != BCM_E_NONE)
    ...
    {
        UI32_T ip_addr = L_STDLIB_Ntoh32(ace_entry->dstip_data);
        UI32_T ip_mask = L_STDLIB_Ntoh32(ace_entry->dstip_mask);

        ret_value = bcm_field_qualify_DstIp(device_id, dev_rule_id, ip_addr, ip_mask);
        if (ret_value != BCM_E_NONE)
...
BOOL_T DEVRM_CreateMeter(UI32_T device_id,UI32_T dev_rule_id, DEVRM_MeterEntry_T *meter_entry, BOOL_T need_reinstalled)
{
    ...
    pol_cfg.pkbits_sec = meter_entry->peak_kbits_sec;
    pol_cfg.ckbits_sec = meter_entry->kbits_sec;

    if (meter_entry->meter_type == DEVRM_POLICER_METER_TYPE_PACKET)
    {
        pol_cfg.flags |= BCM_POLICER_MODE_PACKETS;
        pol_cfg.pkbits_burst = meter_entry->peak_kbytes_burst;
        pol_cfg.ckbits_burst = meter_entry->kbytes_burst;
    }
    else
    {
        pol_cfg.pkbits_burst = meter_entry->peak_kbytes_burst * 8;
        pol_cfg.ckbits_burst = meter_entry->kbytes_burst * 8;
    }

    ret_value = bcm_policer_create(device_id, &pol_cfg, &policer_id);
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_CreateMeter Error! bcm_policer_create[%s]\n", bcm_errmsg(ret_value));
        return FALSE;
    }

    ret_value = bcm_field_entry_policer_attach(device_id, dev_rule_id, 0, policer_id);
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_CreateMeter Error! bcm_field_entry_policer_attach[%s]\n", bcm_errmsg(ret_value));
        return FALSE;
    }

#if 0 /* obsolete by BCM SDK 6, bcmFieldActionMeterConfig is obsolete. */
    /* meter config action */
...
BOOL_T DEVRM_UpdateMeter(UI32_T device_id,UI32_T dev_rule_id, DEVRM_MeterEntry_T *meter_entry)
{
    ...
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_UpdateMeter Error! bcm_field_entry_policer_get[%s]\n", bcm_errmsg(ret_value));
        return FALSE;
    }

    ret_value = bcm_policer_get(device_id, policer_id, &pol_cfg);
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_UpdateMeter Error! bcm_policer_get[%s]\n", bcm_errmsg(ret_value));
        return FALSE;
    }
    ...
    ret_value = bcm_policer_set(device_id, policer_id, &pol_cfg);
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_UpdateMeter Error! bcm_policer_set[%s]\n", bcm_errmsg(ret_value));
        return FALSE;
    }
...
{
    bcm_policer_t           policer_id, org_policer_id;
    int     ret_value = BCM_E_NONE;

    ret_value = bcm_field_entry_policer_get(device_id, src_rule_id, 0, &policer_id);
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_ShareMeter Error! bcm_field_entry_policer_get[%s]\n", bcm_errmsg(ret_value));
        return FALSE;
    }

    ret_value = bcm_field_entry_policer_get(device_id, dst_rule_id, 0, &org_policer_id);
    if(ret_value != BCM_E_NONE)
    {
        org_policer_id = 0;
    }

    ret_value = bcm_field_entry_policer_attach(device_id, dst_rule_id, 0, policer_id);
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_ShareMeter Error! bcm_field_entry_policer_attach[%s]\n", bcm_errmsg(ret_value));
        return FALSE;
    }

    if (org_policer_id != 0)
    {
        ret_value = bcm_policer_destroy(device_id, org_policer_id);
        if(ret_value != BCM_E_NONE && ret_value != BCM_E_BUSY)
        {
            printf("DEVRM_ShareMeter Error! bcm_policer_destroy[%s]\n", bcm_errmsg(ret_value));
            return FALSE;
        }
    }
...
BOOL_T DEVRM_DestroyMeter(UI32_T device_id, UI32_T dev_rule_id)
{
    ...
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_DestroyMeter Error! bcm_field_entry_policer_get[%s]\n", bcm_errmsg(ret_value));
        return FALSE;
    }
    ...
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_DestroyMeter Error! bcm_field_entry_policer_detach[%s]\n", bcm_errmsg(ret_value));
        return FALSE;
    }
    ...
    if(ret_value != BCM_E_NONE && ret_value != BCM_E_BUSY)
    {
        printf("DEVRM_DestroyMeter Error! bcm_policer_destroy[%s]\n", bcm_errmsg(ret_value));
        return FALSE;
    }
...
BOOL_T DEVRM_CreateCounter(UI32_T device_id,UI32_T dev_rule_id, UI32_T counter_mode, BOOL_T need_reinstalled)
{
    ...
    vg_p = _DEVRM_FindVirtualGroupByRuleId(device_id, dev_rule_id);
    if (NULL == vg_p)
    {
        DEVRM_ERR("devId = %lu, dev_rule_id = %lu, virGroup not exist", device_id, dev_rule_id);
        return FALSE;
    }

    /* TODO:
     *   BCM_FIELD_COUNTER_MODE_ is obsolete,
     *   should remove counter_mode and use nstat/stat_arr instead.
     */
    stat_arr[nstat++] = bcmFieldStatPackets;

    ret_value = bcm_field_stat_create(device_id, (bcm_field_group_t)vg_p->group_id, nstat, stat_arr, &stat_id);
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_CreateCounter Error! bcm_field_stat_create[%d]\n",ret_value);
        return FALSE;
    }

    ret_value = bcm_field_entry_stat_attach(device_id, dev_rule_id, stat_id);
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_CreateCounter Error! bcm_field_entry_stat_attach[%d]\n",ret_value);
        return FALSE;
    }

#if 0 /* obsolete by BCM SDK 6, bcmFieldActionUpdateCounter is obsolete. */
...
BOOL_T DEVRM_DestroyCounter(UI32_T device_id, UI32_T dev_rule_id)
{
    ...
    ret_value = bcm_field_entry_stat_get(device_id, dev_rule_id, &stat_id);
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_DestroyCounter Error! bcm_esw_field_entry_stat_get[%d]\n",ret_value);
        return FALSE;
    }

    ret_value = bcm_field_entry_stat_detach(device_id, dev_rule_id, stat_id);
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_DestroyCounter Error! bcm_field_entry_stat_detach[%d]\n",ret_value);
        return FALSE;
    }

    ret_value = bcm_field_entry_destroy(device_id, stat_id);
    if(ret_value != BCM_E_NONE && ret_value != BCM_E_BUSY)
    {
        printf("DEVRM_DestroyCounter Error! bcm_field_entry_destroy[%d]\n",ret_value);
        return FALSE;
    }
    ...
    UI32_T *counter_p)
{
    ...
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_GetCounter Error! bcm_esw_field_entry_stat_get[%d]\n",ret_value);
        return FALSE;
    }

    ret_value = bcm_field_stat_get32(device_id, stat_id, bcmFieldStatPackets, &val);
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_GetCounter Error! bcm_field_stat_get32[%d]\n",ret_value);
        return FALSE;
    }
    ...
    UI32_T dev_rule_id)
{
    ...
    ret_value = bcm_field_stat_set(device_id, stat_id, bcmFieldStatPackets, 0);
    if(ret_value != BCM_E_NONE)
    {
        printf("DEVRM_GetCounter Error! bcm_field_stat_set32[%d]\n",ret_value);
        return FALSE;
    }
...
BOOL_T DEVRM_CopyRule(UI32_T device_id, UI32_T src_dev_rule_id, UI32_T dst_dev_rule_id)
{
    ...
    /* remove meter setting */
    ret_value = bcm_field_entry_policer_get(device_id, src_dev_rule_id, 0, &policer_id);
    if (ret_value == BCM_E_NONE)
    {
        ret_value = bcm_field_entry_policer_detach(device_id, src_dev_rule_id, 0);
        if (BCM_E_NONE != ret_value)
        {
            printf("DEVRM_CopyRule Error! bcm_field_entry_policer_detach[%s]\n", bcm_errmsg(ret_value));
            return FALSE;
        }
        ...
        if ((BCM_E_NONE != ret_value) && (BCM_E_BUSY != ret_value))
        {
            printf("DEVRM_CopyRule Error! bcm_policer_destroy[%s]\n", bcm_errmsg(ret_value));
            return FALSE;
        }
    }
...
static BOOL_T DEVRM_DeleteDevRule(UI32_T device_id, UI32_T dev_rule_id)
{
    ...
    if (policer_id != 0)
    {
        ...
        if (BCM_E_NONE != ret_value)
        {
            printf("DEVRM_DeleteDevRule Error! bcm_field_entry_policer_detach[%s]\n", bcm_errmsg(ret_value));
            return FALSE;
        }
        ...
        if ((BCM_E_NONE != ret_value) && (BCM_E_BUSY != ret_value))
        {
            printf("DEVRM_DeleteDevRule Error! bcm_policer_destroy[%s]\n", bcm_errmsg(ret_value));
            return FALSE;
        }
    }
...
/*------------------------------------------------------------------------------
 * ROUTINE NAME - _DEVRM_FindVirtualGroupByRuleId
 *------------------------------------------------------------------------------
 * PURPOSE  : Find virtual group by device rule ID
 * INPUT    : device_id     - device ID
    ...
    UI32_T dev_rule_id)
{
    ...
    for (i=0; i < devrm_vir_grp[device_id].vsel_num; ++i)
    {
        ...
        if (TRUE == vg_p->valid)
        {
            if (TRUE == _DEVRM_FindRuleIndexByRuleId(&vg_p->rv, dev_rule_id, &rule_idx))
            {
                return vg_p;
            }
        }
