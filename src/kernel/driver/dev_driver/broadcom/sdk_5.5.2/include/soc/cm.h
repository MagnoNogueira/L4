/*
 * $Id: cm.h,v 1.9.6.1 Broadcom SDK $
...
#define	CMWRITE(_d,_a,_data)	\
	((CMVEC(_d).write(&CMDEV(_d).dev, _a, _data)), SOC_E_NONE)
#else
#define	CMREAD(_d,_a)	\
...
#define	CMWRITE(_d,_a,_data)	\
	(CMVEC(_d).base_address ? \
	    ((void)(((VOL uint32 *)CMVEC(_d).base_address)[(_a)/4] = _data), SOC_E_NONE) : \
	    (CMVEC(_d).write(&CMDEV(_d).dev, _a, _data), SOC_E_NONE))
