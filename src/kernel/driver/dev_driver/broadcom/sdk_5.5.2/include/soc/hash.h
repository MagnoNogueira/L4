/*
 * $Id: hash.h,v 1.39.6.1 Broadcom SDK $
...
/* Triumph functions */
extern uint32   soc_tr_l2x_hash(int unit, int hash_sel, int key_nbits, uint8 *key);
extern int      soc_tr_l2x_base_entry_to_key(int unit, uint32 *entry, 
                                             uint8 *key);
extern int      soc_tr_l2x_param_to_key(int unit, sal_mac_addr_t mac, 
                                        int vid, uint8 *key);
...
extern int      soc_tr_vlan_xlate_base_entry_to_key(int unit, uint32 *entry, 
                                                    uint8 *key);
extern uint32   soc_tr_vlan_xlate_entry_hash(int unit, int hash_sel, 
                                             uint32 *entry);
extern uint32   soc_tr_vlan_xlate_bank_entry_hash(int unit, int bank, uint32 *entry);

extern uint32   soc_tr_egr_vlan_xlate_hash(int unit, int hash_sel, int key_nbits, 
                                           uint8 *key);
extern int      soc_tr_egr_vlan_xlate_base_entry_to_key(int unit, uint32 *entry, 
                                                        uint8 *key);
extern uint32   soc_tr_egr_vlan_xlate_entry_hash(int unit, int hash_sel, 
                                                 uint32 *entry);
extern uint32   soc_tr_egr_vlan_xlate_bank_entry_hash(int unit, int bank, uint32 *entry);

extern uint32   soc_tr_mpls_hash(int unit, int hash_sel, int key_nbits, 
                                 uint32 *base_entry, uint8 *key);
extern int      soc_tr_mpls_base_entry_to_key(int unit, uint32 *entry, uint8 *key);
...
/* These two definitions may be expanded later as necessary*/
#define SOC_HASH_VRF_BITS(unit)    \
        (SOC_IS_FIREBOLT2(unit) || SOC_IS_RAVEN(unit) ? FB2_HASH_VRF_BITS : TRX_HASH_VRF_BITS)
#define SOC_HASH_VRF_MASK(unit)    \
        (SOC_IS_FIREBOLT2(unit) || SOC_IS_RAVEN(unit) ? FB2_HASH_VRF_MASK : TRX_HASH_VRF_MASK)
