/*
 * $Id: brcm_osl.h,v 1.2 Broadcom SDK $
...
/* packet primitives */
#define	ET_PKTGET(dev, len, send)	        et_pktget(dev, len, send)
extern void *et_pktget(void *dev, uint len, bool send);
#define	ET_PKTFREE(dev, dv, send)		et_pktfree(dev, dv)
extern void et_pktfree(void *dev, void *dv);
#define	ET_PKTDATA(dev, dv)       ((uint8 *) \
		                ((((robo_dv_t*)dv)->dv_dcb) \
			         [((robo_dv_t*)dv)->dv_dcnt].dcb_vaddr))
#define	ET_PKTLEN(dev, dv)	       ((((robo_dv_t*)dv)->dv_dcb) \
		                [((robo_dv_t*)dv)->dv_dcnt].len)
#define	ET_PKTNEXT(dev, dv)       ((((robo_dv_t*)dv)->dv_dcb) \
		                [((robo_dv_t*)dv)->dv_dcnt++].next?dv:NULL)
#define	ET_PKTSETLEN(dev, dv, l)  (((((robo_dv_t*)dv)->dv_dcb) \
			        [((robo_dv_t*)dv)->dv_dcnt].len)=(l))
