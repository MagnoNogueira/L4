/*
 * $Id: l2x.h,v 1.35.6.3 Broadcom SDK $
...
extern int soc_l2x_port_age(int unit, uint32 age_rval, soc_reg_t alt_age_reg,
                            uint32 alt_age_rval);
...
extern int soc_l2x_sync_delete(int unit, uint32 *del_entry, int index);
extern int soc_l2x_sync_delete_by_port(int unit, int mod, 
                                       soc_port_t port, uint32 flags);
extern int soc_l2x_sync_delete_by_mac(int unit, 
                                      sal_mac_addr_t mac, uint32 flags);
extern int soc_l2x_sync_delete_by_vlan(int unit, 
                                       vlan_id_t vid, uint32 flags);
extern int soc_l2x_sync_delete_by_trunk(int unit, 
                                        int tid, uint32 flags);
extern int soc_l2x_sync_delete_by_mac_port(int unit, sal_mac_addr_t mac,
				           int mod, soc_port_t port,
				           uint32 flags);
extern int soc_l2x_sync_delete_by_vlan_port(int unit, vlan_id_t vid,
				            int mod, soc_port_t port, 
                                            uint32 flags);
extern int soc_l2x_sync_delete_by_vfi(int unit, int vfi, uint32 flags);
...
extern int soc_l2x_frozen_cml_set(int unit, soc_port_t port, int cml,
				  int *repl_cml);
extern int soc_l2x_frozen_cml_get(int unit, soc_port_t port, int *cml);
...
#define SOC_L2X_MEM_LOCK(unit)\
    ...
    } else if (soc_mem_is_valid(unit, ARLm)) {\
                soc_mem_lock(unit, ARLm);\
...
#define SOC_L2X_MEM_UNLOCK(unit)\
    if (soc_feature(unit, soc_feature_l2_multiple)) {\
        ...
        soc_mem_unlock(unit, ARLm);\
    } else { soc_mem_unlock(unit, L2Xm); }

#else 

#define SOC_L2X_MEM_LOCK(unit)\
        if  (soc_mem_is_valid(unit, ARLm)) {\
            soc_mem_lock(unit, ARLm);\
        } else {soc_mem_lock(unit, L2Xm);}
