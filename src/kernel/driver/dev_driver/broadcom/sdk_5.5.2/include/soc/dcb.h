...
#define SOC_DCB_RX_REASONS_GET(_u, _dcb, _reasons)  \
        soc_dcb_rx_reasons_get(_u, _dcb, _reasons)
...
extern void     soc_dcb_rx_reasons_get(int unit, dcb_t *dcb, 
                                       soc_rx_reasons_t *reasons);
#endif

/*
 * Typedef: ROBO DMA Control Block (dcb)
 * Purpose: Keep the control information of Tx/Rx frames
 */

typedef struct robo_dcb_s {
    struct robo_dcb_s *next;                 /* ptr to next robo_dcb_t in a packet */
