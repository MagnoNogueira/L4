/*
 * $Id: system.h,v 1.34.6.1 Broadcom SDK $
...
#define FORMAT_PBMP_MAX   148   /* "0xdeadbeefdeadbeef; fe0,fe2,....ge2,hg" */
 ...
 extern int     parse_macaddr(char *str, mac_addr_t macaddr);
 extern void    format_macaddr(char buf[MACADDR_STR_LEN], mac_addr_t macaddr);
 extern void    increment_macaddr(mac_addr_t macaddr, int amount);
 ...
 extern int     parse_pbmp(int unit, char *s, pbmp_t *pbmp);
 extern int     parse_pbmp_no_error(int unit, char *s, pbmp_t *pbmp);
 extern int     format_pbmp(int unit, char *buf, int bufsize, pbmp_t pbmp);
 ...
 extern int     parse_block_range(int unit, soc_block_t regtype, char *s,
                          int *min, int *max, soc_block_t *mask);
 extern int     parse_memory_name(int unit, soc_mem_t *result, char *str, int *copyno);
...
/* Packet Watcher */
 ...
 extern int pw_stop(int unit, int sync);
...
#define UNSUPPORTED_COMMAND(unit, chiptype, a)\
    if (SOC_CHIP_GROUP(unit) == chiptype) { \
        soc_cm_debug(DK_ERR, "%s: command unsupported on %s\n",\
...
#define SHOW_CTR_ED             0x40
 ...
 extern int     do_show_counters(int unit, soc_reg_t ctr_reg, pbmp_t pbmp,
                         int flags);


 extern int     do_resync_counters(int unit, pbmp_t pbmp);

 extern void    counter_val_set_by_port(int unit, pbmp_t pbmp, uint64 val);
