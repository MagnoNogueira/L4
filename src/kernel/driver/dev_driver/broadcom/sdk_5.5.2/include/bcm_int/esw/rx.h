/*
 * $Id: rx.h,v 1.29.6.1 Broadcom SDK $
...
/* Some default values */
#define RX_PKT_SIZE_DFLT       (13 * 1024)    /* 12 K packets, plus spare */
#define RX_PPC_DFLT            4             /* 4 pkts/chain */
#define RX_PPS_DFLT            1000          /* 1000 pkts/sec */
#define RX_CHAINS_DFLT         4             /* 4 chains */
...
/* As above, but lock (for remote, non-interrupt units) */
#define _Q_ENQUEUE_LOCK(q, pkt)                          \
    do {                                                 \
        _Q_LOCK(q);                                      \
        (pkt)->_next = NULL;                             \
        if (_Q_EMPTY(q)) (q)->head = (q)->tail = (pkt);  \
...
#define BCM_RX_POOL_BYTES_DEFAULT      8192    /* 0x600 */
#endif

#ifdef BCM_WARM_BOOT_SUPPORT
extern int _bcm_rx_shutdown(int unit);
#else
#define _bcm_rx_shutdown(u)   (BCM_E_NONE)
