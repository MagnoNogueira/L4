/*
 * $Id: l3.h,v 1.51.6.2 Broadcom SDK $
...
#define BCM_TTL_VALID(_ttl_) (((_ttl_) >= 0) && ((_ttl_) <= 0xff))
#define BCM_PRIORITY_VALID(_pri_) (((_pri_) >= 0) && ((_pri_) <= 0x7))
...
#define _BCM_PRINT_IP6_ADDR(flags, ipaddr) \
    BCM_DEBUG((flags | BCM_DBG_L3), \
...
#define _BCM_PRINT_IP4ADDR(flags, ipaddr) \
    BCM_DEBUG((flags | BCM_DBG_L3), \
	("%d.%d.%d.%d\n", \
        ((ipaddr)>>24)&0xFF, ((ipaddr)>>16)&0xFF, \
        ((ipaddr)>>8)&0xFF, ((ipaddr))&0xFF))

#define _BCM_PRINT_IP6_NET(flags, ipaddr, masklen) \
    BCM_DEBUG((flags | BCM_DBG_L3), \
...
#define _BCM_PRINT_IP4_NET(flags, ipaddr, masklen) \
    BCM_DEBUG((flags | BCM_DBG_L3), \
	("%d.%d.%d.%d/%d\n", \
        ((ipaddr)>>24)&0xFF, ((ipaddr)>>16)&0xFF, \
        ((ipaddr)>>8)&0xFF, (ipaddr)&0xFF, \
...
#define _BCM_TUNNEL_OUTER_HEADER_IPV6(_tnl_type_)    \
       ((((_tnl_type_) == bcmTunnelTypeIp4In6)    || \
         ...
         ((_tnl_type_) == bcmTunnelTypePim6SmDr2)) ? TRUE : FALSE)
