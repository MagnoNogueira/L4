...
#define REG_ADD(unit, port, reg, val)					   \
    if (SOC_REG_IS_VALID(unit, reg) && SOC_REG_IS_COUNTER(unit, reg)) {	   \
        uint64 reg_val;							   \
        SOC_IF_ERROR_RETURN(soc_robo_counter_get(unit, port, reg,               \
                                            0, &reg_val));                 \
...
extern int bcm_stat_reinit(int unit);
...
extern int bcm_stat_sync(int unit);

extern int bcm_stat_get(int unit, bcm_port_t port, 
			bcm_stat_val_t type, uint64 *value);

extern int bcm_stat_get32(int unit, bcm_port_t port, 
			  bcm_stat_val_t type, uint32 *value);

extern int _bcm_stat_get(int unit, bcm_port_t port, 
