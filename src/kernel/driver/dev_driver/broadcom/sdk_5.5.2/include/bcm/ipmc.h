/*
 * $Id: ipmc.h,v 1.33.6.3 Broadcom SDK $
 * 
...
typedef struct bcm_ipmc_addr_s {
    ...
    int ttl;                            /* TTL threshold (Strata only; per-port
                                           on XGS). */
    int v;                              /* Valid bit. */
    int cd;                             /* CPU or drop bit if src port mismatch
                                           (Strata only; per-port on XGS). */
...
#define bcm_ipmc_addr_init      bcm_ipmc_addr_t_init 
...
/* Set the source port mismatch rule (API superceded). */
extern int bcm_ipmc_sport_mismatch_action(
    int unit, 
    bcm_ip_t s_ip_addr, 
    bcm_ip_t mc_ip_addr, 
    bcm_vlan_t vid, 
...
/* Set the TTL of an IPMC entry (API superceded). */
extern int bcm_ipmc_ttl_threshold_set(
    int unit, 
    bcm_ip_t s_ip_addr, 
    bcm_ip_t mc_ip_addr, 
    bcm_vlan_t vid, 
