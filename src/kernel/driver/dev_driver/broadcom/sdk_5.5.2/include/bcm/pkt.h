/*
 * $Id: pkt.h,v 1.45.2.5 Broadcom SDK $
 * 
...
struct bcm_pkt_s { 
    ...
    uint32 rx_timestamp;            /* Time stamp of time sync protocol packets. */
    ...
    bcm_pkt_t *_last_pkt;           /* To link to end of linked lists
                                       (internal). */
    void *_dv;                      /* DV controlling this packet (internal). */
    int8 _idx;                      /* Packet's index in the DV for RX
                                       (internal). */
    bcm_pkt_t *_next;               /* For BCM layer linked lists (internal). */
    void *alloc_ptr;                /* Pointer for deallocation (internal). */
    void *trans_ptr;                /* Transport pointer associated with packet
                                       (internal). */
    uint8 _higig[16];               /* HiGig header value (network byte order). */
    uint8 _pb_hdr[12];              /* Pipe Bypass Header (network byte order). */
    uint8 _sltag[4];                /* SL tag value (network byte order). */
    uint8 _vtag[4];                 /* VLAN tag if not in packet (network byte
                                       order). */
    uint8 _vtag2[4];                /* Second VLAN tag if not in packet (network
                                       byte order). */
...
#define BCM_PKT_TX_LEN(pkt, bytes)  \
    do { \
        ...
        if ((pkt)->flags & BCM_PKT_F_CRC_ALLOC) (bytes) += 4; \
    } while (0) 
...
#define BCM_PKT_HAS_VLAN2(pkt)             /* Look at type fields in VLAN tags
                                              and check if more present. */
 ...
 * as best as possible.
 * 
 * If size > 0, then a packet buffer should be allocated and the packet
 * will be setup up for a single data block. If size <= 0, no packet
