...
typedef struct ibde_s {
    ...
    /* Support SWITCH or ETHERNET devices */
    int     (*num_devices)(int type);
#define BDE_ALL_DEVICES           0x0
#define BDE_SWITCH_DEVICES        0x1
#define BDE_ETHER_DEVICES         0x2
...
#define BDE_PCI_DEV_TYPE      SAL_PCI_DEV_TYPE    /* Type of PCI device */
#define BDE_SPI_DEV_TYPE      SAL_SPI_DEV_TYPE    /* Type of SPI device
						     accessed via SMP */
#define BDE_EB_DEV_TYPE      SAL_EB_DEV_TYPE      /* Type of EB device */
#define BDE_ICS_DEV_TYPE      SAL_ICS_DEV_TYPE      /* Type of EB device */

#define BDE_DEV_BUS_TYPE_MASK   SAL_DEV_BUS_TYPE_MASK
    
#define BDE_SWITCH_DEV_TYPE   SAL_SWITCH_DEV_TYPE /* Type of switch device */
#define BDE_ETHER_DEV_TYPE    SAL_ETHER_DEV_TYPE  /* Type of outband ethernet
						     device */
...
#define BDE_DEV_BUS_WR_16BIT    SAL_DEV_BUS_WR_16BIT
    ...
    int  (*i2c_read)(int d, uint16 addr, uint8 *buf, int len);
    int  (*i2c_write)(int d, uint16 addr, uint8 *buf, int len);
    int  (*i2c_read_intr)(int d, uint8 chipid, uint8 *buf, int len);
    int  (*i2c_read_ARA)(int d, uint8 *chipid, int len);
