/*
 * $Id: lplist.h,v 1.9 Broadcom SDK $
 ...
 *
 * File:        lplist.h
 * Purpose:     Logical port list type declarations
 * Note:        Please use accessors as the implementation of
 *              of LPList may change with the new BCM dispatch
 *              layer.
 */

#ifndef   _BCMX_LPLIST_H_
#define   _BCMX_LPLIST_H_

#include <bcm/types.h>
#include <bcmx/types.h>


/* Deprecated flags to indicate groups of ports. Use BCMX_PORT_LP_* instead. */
...
typedef struct bcmx_lplist_s {
    int lp_last;              /* Index of last elt on list. -1 --> empty */
...
/* Alias bcmx_lplist_add as a macro. */
#define BCMX_LPLIST_ADD(list, lport) bcmx_lplist_add(list, lport)

/* NOTE:  List is a pointer here; not a pointer below.  Ick. */
#define BCMX_LPLIST_IDX_ITER(list_p, lport, count)               \
    for ((lport) = (list_p)->lp_ports[0], (count) = 0;           \
         (count) <= (list_p)->lp_last;                           \
         (lport) = (list_p)->lp_ports[++(count)])
...
extern int bcmx_lplist_init(bcmx_lplist_t *list, int init_count, uint32 flags);
extern int bcmx_lplist_free(bcmx_lplist_t *list);
extern int bcmx_lplist_clear(bcmx_lplist_t *list);
extern void bcmx_lplist_t_init(bcmx_lplist_t *list);
extern void bcmx_lplist_t_free(bcmx_lplist_t *list);

extern int bcmx_lplist_index_get(bcmx_lplist_t *list, bcmx_lport_t port);
extern int bcmx_lplist_index_get_from(bcmx_lplist_t *list,
                                      int position, bcmx_lport_t port);
extern bcmx_lport_t bcmx_lplist_index(bcmx_lplist_t *list, int position);
...
extern int bcmx_lplist_add(bcmx_lplist_t *list, bcmx_lport_t lport);

extern int bcmx_lplist_port_remove(bcmx_lplist_t *list,
                                   bcmx_lport_t lport, int all);

extern int bcmx_lplist_eq(bcmx_lplist_t *list1, bcmx_lplist_t *list2);

/* List operations */
extern int bcmx_lplist_append(bcmx_lplist_t *list1, bcmx_lplist_t *list2);
extern int bcmx_lplist_copy(bcmx_lplist_t *dest, bcmx_lplist_t *src);

extern int bcmx_lplist_check(bcmx_lplist_t *list);

extern int bcmx_lplist_range(bcmx_lplist_t *list, bcmx_lport_t start,
                             bcmx_lport_t end);


extern int bcmx_lplist_is_null(bcmx_lplist_t *list);
extern int bcmx_lplist_is_empty(bcmx_lplist_t *list);
extern int bcmx_lplist_count(bcmx_lplist_t *list);
extern void bcmx_lplist_remove(bcmx_lplist_t *list, int lport);
extern void bcmx_lplist_idx_remove(bcmx_lplist_t *list, int idx);
extern int bcmx_lplist_pbmp_add(bcmx_lplist_t *list, int unit, bcm_pbmp_t *pbm);
extern void bcmx_lplist_to_pbmp(bcmx_lplist_t *list, int unit, bcm_pbmp_t *pbm);
extern int bcmx_lplist_sort(bcmx_lplist_t *list);
extern int bcmx_lplist_uniq(bcmx_lplist_t *list);
...
extern int _bcmx_lplist_pbmp_add(bcmx_lplist_t *list,
