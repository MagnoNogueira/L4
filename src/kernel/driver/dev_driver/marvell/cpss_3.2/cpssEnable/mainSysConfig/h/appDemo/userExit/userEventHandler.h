...
#else /* care event per accton's requiremnt */
#define APP_DEMO_P0_UNI_EV_DEFAULT { }
#endif                                    
...
#define APP_DEMO_P7_UNI_EV_DEFAULT { CPSS_PP_EB_AUQ_PENDING_E                   \
                                    ...
                                    ,CPSS_PP_MAC_MESSAGE_TO_CPU_READY_E         \
                                                                                \
/* the event of CPSS_PP_MAC_AGE_VIA_TRIGGER_ENDED_E must be lst one on this */  \
/* list because we will remove it on ExMx systems                           */  \
                                    ,CPSS_PP_MAC_AGE_VIA_TRIGGER_ENDED_E        \
                                    }
#else /* care event per accton's requiremnt */
#define APP_DEMO_P7_UNI_EV_DEFAULT { CPSS_PP_EB_SECURITY_BREACH_UPDATE_E }
#endif


#if 0 /* original */
#define APP_DEMO_P8_UNI_EV_DEFAULT { CPSS_PP_TX_END_E                           \
                                    ,CPSS_PP_TX_ERR_QUEUE_E                     \
                                    }
#else /* care event per accton's requiremnt */
#define APP_DEMO_P8_UNI_EV_DEFAULT { }
#endif        
...
#else /* care event per accton's requiremnt */
#define APP_DEMO_P9_UNI_EV_DEFAULT { }
#endif
...
/* set of 4 pointers to functions that need to be called instead of directly to
    the CPSS APIs.

    those 4 pointers allow to implement a dispatcher between the CPSS and the
    "appdemo emulate the CPSS events"(emulation needed for Rx/Tx in device with
    Ethernet port connected to the CPU)


*/
extern APP_DEMO_CPSS_EVENT_BIND_FUNC            appDemoCpssEventBind;
extern APP_DEMO_CPSS_EVENT_SELECT_FUNC          appDemoCpssEventSelect;
extern APP_DEMO_CPSS_EVENT_RECV_FUNC            appDemoCpssEventRecv;
extern APP_DEMO_CPSS_EVENT_DEVICE_MASK_SET_FUNC appDemoCpssEventDeviceMaskSet;
