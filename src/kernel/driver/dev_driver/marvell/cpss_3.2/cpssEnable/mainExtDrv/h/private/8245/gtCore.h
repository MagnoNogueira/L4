...
*       the SDRAM, devices, GT`s internal registers and PCI (using the PCI`s
...
#ifndef GT_REG_WRITE
    /* alex, to support GCC CROSS COMPILER FOR SDK 2.1, gcc 4.2.1, ORION/KIRKWOOD */
    /* this define specific for ppc-fbsd-e500-gcc needed to avoid compilation 
     * error while using this macro: "invalid lvalue in assignment"
     */
    #define GT_REG_WRITE(offset, data)                                      \
    {                                                                       \
        volatile unsigned int *dummyPtr;                                    \
        dummyPtr = (unsigned int *)(NONE_CACHEABLE(gtInternalRegBaseAddr)   \
                                    | (offset));                            \
        *dummyPtr = GT_WORD_SWAP(data);                                     \
    }
#endif    
