...
typedef struct
{
    ...
    GT_U32                          busBaseAddr[CPSS_MAX_PORT_GROUPS_CNS];
    GT_U32                          internalPciBase[CPSS_MAX_PORT_GROUPS_CNS];
...
/* flag that state the DB of cpssDriver was released and now we are after that stage
    NOTE: the flag start with GT_FALSE , but once changed to GT_TRUE it will stay
          that way forever.
*/
extern GT_BOOL  prvCpssDrvAfterDbRelease;
