 ...
 *     cpuEtherPortInfo - Holds CPU Ether port configuration parameters
...
typedef struct
{
    ...
    PRV_CPSS_GEN_NETIF_CPU_ETHER_PORT_INFO_STC cpuEtherPortInfo;
...
/* array of pointers to application supplied info about HA for the PPs */
extern PRV_CPSS_PP_HA_CONFIG_INFO_STC *cpssPpHaDevData[PRV_CPSS_MAX_PP_DEVICES_CNS];


/* flag that state the DB of cpss was released and now we are after that stage
    NOTE: the flag start with GT_FALSE , but once changed to GT_TRUE it will stay
          that way forever.
*/
extern GT_BOOL  prvCpssAfterDbRelease;
...
*
* INPUTS:
*       sysCfgPhase1InfoPtr - (pointer to) cpss Phase1 system level specific
*                               parameters.
*
* OUTPUTS:
*       None.
...
*
*
*  APPLICABLE DEVICES: ALL devices must call this function
*
* INPUTS:
