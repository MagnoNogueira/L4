 ...
 *              Relevant for DxChXcat and above devices only.
 *
 *  Comments:
 *
 */
 ...
 *                      Supported only DXCH2 and above.
 ...
 *                       Supported DxCh3 and above devices.
 ...
 *                DxChXcat and above devices support all values of the enum
 ...
 *                  Not supported for DxChXcat and above.
 ...
 *                  L3 header minus 2. Ethernet type of IP.
 *                  Supported only for DxChXcat and above.
 *    CPSS_DXCH_PCL_OFFSET_MPLS_MINUS_2_E   - offset from
 *                  ethernet type of MPLS minus 2.
 *                  Supported only for DxChXcat and above.
 *    CPSS_DXCH_PCL_OFFSET_INVALID_E - invalid UDB - conains 0 always
 *                  Supported only for DxChXcat and above.
 *
 * Comment:
 ...
 *      For DxChXCat and above devices there are 23 UDBs for each of 8 packet type
 ...
 *
 * Description: Packet types for DxChXCat and above devices
 *
 * Enumerations:
 ...
 *    CPSS_DXCH_PCL_PACKET_TYPE_UDE_E            - User Defined Ethernet Type
 ...
 *      Only for DxChXCat and above devices.
 *
 * Fields:
 *      Each value is BOOL,
 *      GT_TRUE causes overriding some User Defined Bytes in some key.
 *          by VRF-ID's MSB or USB or Qos Prfile index as listed below.
 ...
 *                  For DxChXCat and above devices UDBs 15,16,17
 *
 *  Comments:
 */
typedef struct
 ...
 *                         For DxChXCat and above devices UDBs 18,19
 *
 *  Comments:
 ...
 *     udb         - User Defined Bytes with indexes 0,1,2
 *                   For DxChXCat and above devices UDBs 20,21,22
 *
 *  Comments:
 *  udb[0] may contain the TCP/UDP comparator result
 */
 ...
 *                   For DxChXCat and above devices UDBs also 0,1,2,3,4,5
 *
 *  Comments:
 ...
 *     macSa       - Ethernet Source MAC address.
 *     udb         - User Defined Bytes with indexes 0,1,2,3,4,5
 *                   For DxChXCat and above devices UDBs 6,7,8,9,10,11
 *
 *  Comments:
 *  udb[2] may contain the TCP/UDP comparator result
 */
 ...
 *     isIpv6HopByHop    - see in CPSS_DXCH_PCL_RULE_FORMAT_STD_IP_L2_QOS_STC
 *     udb         - User Defined Bytes with indexes 0,1,2
 *                   For DxChXCat and above devices UDBs 12,13,14
 *
 *  Comments:
 *  udb[2] may contain the TCP/UDP comparator result
 */
 ...
 *              DxChXCat and above devices only
 *
 * Fields:
 * pclId            - The PCL-ID attribute assigned to this lookup.
 * macToMe          - MAC To Me
 ...
 *                             For DxChXcat and above devices lookup0
 *                                 GT_TRUE - enable Lookup0_0
 *                                 GT_FALSE - disable Lookup0_0
 *    pclId                  - PCL-ID bits in the TCAM search key.
 *    dualLookup             - Relevant for lookup 0 only.
 *                             For DxCh3 devices
 ...
 *                             For DxChXcat and above devices and above
 *                                 GT_TRUE -  lookup0_1 enabled.
 *                                 lookup0_1 uses pclIdL01.
 *                                 GT_FALSE - lookup0_1 disabled.
 *    pclIdL01               - only for DxChXcat and above devices
...
*                          The TCAM has up to 3K/4 (768) rows.
*                          See datasheet of particular device for TCAM size
*                          Each row can be used as:
*                              - 4 standard rules
*                              - 2 extended rules
...
*                          The TCAM partitioning is as follows:
*                          - Standard rules.
*                            Rule index may be in the range from 0 up to 3K (3072).
*                          - Extended rules.
*                            Rule index may be in the range from 0 up to 1.5K (1536).
*                            Extended rule consumes the space of two standard
*                            rules:
...
*
* APPLICABLE DEVICES:  DxCh1, DxCh2 Devices
*
* INPUTS:
*       devNum       - Device number.
...
*
* APPLICABLE DEVICES:  DxCh1 Devices
*
* INPUTS:
*    devNum               - device number
...
*       Old function for DXCH, DXCH2 devices and for
...
*                      DxChXcat and above supports the parameter
*                      DxChXcat and above devices has separate HW entries
