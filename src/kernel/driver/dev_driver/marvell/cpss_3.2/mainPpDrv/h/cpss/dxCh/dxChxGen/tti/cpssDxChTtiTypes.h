 ...
 *                          IPv4_OVER_GRE_IPv4
 *                          IPv6_OVER_GRE_IPv4
 *                          Unknown - If the packet is identified as IPv4 GRE tunnel and
 *                                    there is no match on the configured GRE protocols
 *                          Any Protocol - the packet is not one of the above protocols
 ...
 *  mpls      - MPLS rule
 *  eth       - ethernet rule
 *  mim       - Mac in Mac rule (DxChXcat and above)
 *
 */
typedef union
{
 ...
 *      CPSS_DXCH_TTI_REDIRECT_TO_ROUTER_LOOKUP_E - policy routing
 *      CPSS_DXCH_TTI_VRF_ID_ASSIGN_E             - vrf id assignment
 */
typedef enum
{
 ...
 *                               - DxChXcat and above devices support index 0,4,8,12…(max IP TCAM row * 4) only
 ...
 * Fields:
 *
 *  type1      - type1 action (DxCh3)
 *  type2      - type2 action (DxChXcat and above)
 *
 */
typedef union
{
 ...
 * Enumerations:
 *
 *      CPSS_DXCH_TTI_ACTION_TYPE1_ENT    - type1 action (Ch3)
 *      CPSS_DXCH_TTI_ACTION_TYPE2_ENT    - type2 action (xCat and above)
 *
 */
typedef enum
{
