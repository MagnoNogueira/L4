...
/*
* typedef: enum CPSS_DXCH_PORT_CN_FRAME_FORMAT_ENT
*
* Description: Enumeration of Congestion Notification (CN) frame
*  format generated/identified by the device.
*
* Enumerations:
*  CPSS_DXCH_PORT_CN_FRAME_ECM_E - ECM
*  CPSS_DXCH_PORT_CN_FRAME_QCN_CCFC_E  - QCN/CCFC
*
* Comments:
*         None.
*/ 
typedef enum
{
    CPSS_DXCH_PORT_CN_FRAME_ECM_E,
    CPSS_DXCH_PORT_CN_FRAME_QCN_CCFC_E
}CPSS_DXCH_PORT_CN_FRAME_FORMAT_ENT;
 ...
 *      severeThreshold      - severe threshold in buffers. Relevant only in ECM mode.
 *      mildThreshold        - mild threshold in buffers.Relevant only in ECM mode.
 *      sampleScaleThreshold - sample scale threshold in buffers. Relevant only in ECM mode.
 *
 * Comments:
 *      None.
 */
typedef struct {
    GT_U32 threshold;
    GT_U32 severeThreshold;
    GT_U32 mildThreshold;
    GT_U32 sampleScaleThreshold;
 ...
 *                         untagged.(0..0xFFF)
 *      negateQoffset    - GT_FALSE: Keep the sign of Qoffset
 *                         GT_TRUE: Negate the sign of Qoffset
 ...
 *                         the device. (0..0xF)
 *      cpidMsb          - CNM-header<CPID[31:10]> field for CNMs generated
 *                         by the device.
 ...
 *      qSaturatedUpdEnable - Enables update the value of <Q_Saturated> field in the CN header
 *                         GT_FALSE - Disable: <Q_Saturated> = 0.
 *                         GT_TRUE - Enable: Reflects the saturation state of the queue
 *                         calculated by ECM triggering logic.
 *                           Relevant only in ECM mode.
 *      removeCmTag      - GT_FALSE: Keep CM Tag in ECM payload.
 *                         GT_TRUE: Remove CM Tag in ECM payload.
 *                         Relevant only in ECM mode
 *      logScale         - The Log scale is used for ECM frames generated by the device.
 *                         Relevant only in ECM mode. (-3..3)                  
...
typedef struct {
    ...
    GT_BOOL  negateQoffset;
    GT_U32   scaleFactor;
    GT_U32   version;
    GT_U64   cpidMsb;
    GT_BOOL  cnUntaggedEnable;
    GT_BOOL  qSaturatedUpdEnable;
    GT_BOOL  removeCmTag;
    GT_32    logScale;
    GT_BOOL  forceCnTag;
    GT_U32   flowIdTag;
...
*       2. To bind port to CN profile in Lion use 
...
*
* COMMENTS:
*       1. DxChXcat: Panic Pause uses Port Speed Index 0 and 7 to indicate
*          XON and XOFF. Port timer configurations 0 and 7 must be configured
*          to 0x0 and 0xFFFF if Panic Pause is used.
*       2. DxChXcat: index is always congested port speed index (CCFC mode).
*       3. Lion: index determined by cpssDxChPortCnPrioritySpeedLocationSet.
*
*******************************************************************************/
...
*
* COMMENTS:
*       1. DxChXcat: index is always congested port speed index (CCFC mode).
*       3. Lion: index determined by cpssDxChPortCnPrioritySpeedLocationSet.
*
*******************************************************************************/
...
/*******************************************************************************
* cpssDxChPortCnFrameFormatSet
*
* DESCRIPTION:
*       Sets Congestion Notification (CN) frame format.
*
* APPLICABLE DEVICES: Lion and above
*
* INPUTS:
*       devNum     - device number.
*       cnFormat   - CN frame format generated/identified by the device.
*
* OUTPUTS:
*       None
*
...
*
*******************************************************************************/
GT_STATUS cpssDxChPortCnFrameFormatSet
(
    IN GT_U8                               devNum,
    IN CPSS_DXCH_PORT_CN_FRAME_FORMAT_ENT  cnFormat
);

/*******************************************************************************
* cpssDxChPortCnFrameFormatGet
*
* DESCRIPTION:
*       Gets Congestion Notification (CN) frame format.
*
* APPLICABLE DEVICES: Lion and above
*
* INPUTS:
...
*
* OUTPUTS:
*       cnFormatPtr - (pointer to) CN frame format generated/identified 
*                                  by the device.
*
* RETURNS:
*       GT_OK                    - on success
...
*
*******************************************************************************/
GT_STATUS cpssDxChPortCnFrameFormatGet
(
    IN  GT_U8    devNum,
    OUT CPSS_DXCH_PORT_CN_FRAME_FORMAT_ENT  *cnFormatPtr
);
...
*
* DESCRIPTION:
*       Sets CN prifle configurations for given tc queue.
*
* APPLICABLE DEVICES: Lion and above
*
* INPUTS:
...
*
* DESCRIPTION:
*       Gets CN prifle configurations for given tc queue.
*
* APPLICABLE DEVICES: Lion and above
*
* INPUTS:
...
/*******************************************************************************
* cpssDxChPortCnCmTagEtherTypeSet
*
* DESCRIPTION:
*       Sets CM-tag identification configurations.
*
* APPLICABLE DEVICES: Lion and above.
*
* INPUTS:
*       devNum      - device number.
*       identEnable - GT_TRUE: identification of CM-tagged frames enabled.
*                     GT_FALSE: identification of CM-tagged frames disabled.
*       etherType   - The EtherType that identifies CM-tagged frames (0..0xFFFF).
*                     Only relevant if identEnable==GT_TRUE
*
* OUTPUTS:
*       None.
*
...
*
* COMMENTS:
*       CM-tag should be identified only when working in ECM mode.
*
*******************************************************************************/
GT_STATUS cpssDxChPortCnCmTagEtherTypeSet
(
    IN GT_U8    devNum,
    IN GT_BOOL  identEnable,
    IN GT_U32   etherType
);

/*******************************************************************************
* cpssDxChPortCnCmTagEtherTypeGet
*
* DESCRIPTION:
*       Gets CM-tag identification configurations.
*
* APPLICABLE DEVICES: Lion and above.
*
* INPUTS:
...
*
* OUTPUTS:
*       identEnablePtr - (pointer to) CM-tag identification status
*                     GT_TRUE: identification of CM-tagged frames enabled.
*                     GT_FALSE: identification of CM-tagged frames disabled.
*       etherTypePtr - (pointer to) EtherType for CN frames.
*
* RETURNS:
...
*
*******************************************************************************/
GT_STATUS cpssDxChPortCnCmTagEtherTypeGet
(
    IN  GT_U8    devNum,
    OUT GT_BOOL  *identEnablePtr,
    OUT GT_U32   *etherTypePtr
);
