 ...
 *                              relevant to 'non multi-port-groups' device.
 ...
 *   isExternalCpuConnected - it's ignored for all not DxChXcat devices
 *                            it's ignored for DxChXcat devices without internal CPU
 *                            it's relevant only for DxChXcat devices with internal CPU
 ...
 *          a. netIfCfg : Cpss use all SMDA relate allocation for 'first active port group'
 *             since only this port group will be used for rx/tx traffic from/to cpu.
 *          b. auqCfg : Cpss divide the DMA space evenly between all active port groups.
 ...
 *             port groups (to support CNC , and also for B0 with 'split FDB mode')
 */
typedef struct
{
    GT_U8                   newDevNum;
