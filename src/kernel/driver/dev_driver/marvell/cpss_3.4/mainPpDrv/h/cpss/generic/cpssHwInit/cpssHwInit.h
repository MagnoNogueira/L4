...
/*
 * Typedef enum CPSS_HSU_STATE_ENT
 *
 * Description:
 *      Indicates in which state HSU process is.
 *
 *      CPSS_HSU_STATE_IN_PROGRESS_E  - HSU is going on
 *      CPSS_HSU_STATE_COMPLETED_E    - HSU is completed.
 */

typedef enum
{
    CPSS_HSU_STATE_IN_PROGRESS_E,
    CPSS_HSU_STATE_COMPLETED_E
} CPSS_HSU_STATE_ENT;


/*
 * Typedef: struct CPSS_HSU_INFO_STC
 *
 * Description: This struct containes information about HSU process.
 *
 * Fields:
 *      hsuState          - Indicates in which state HSU process is
 */
typedef struct
{
    CPSS_HSU_STATE_ENT hsuState;
} CPSS_HSU_INFO_STC;

/* define the HSU  global DB */
extern CPSS_HSU_INFO_STC   hsuInfo;
