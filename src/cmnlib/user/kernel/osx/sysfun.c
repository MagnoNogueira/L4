...
typedef enum
{
    SYSFUN_TIMER_RUNNING,
    SYSFUN_TIMER_STOP
} SYSFUN_TimerState_T;

typedef struct
{
    periodic_timer_cb_t     timer_cb;
    struct timespec         it_interval;    /* config value */
    struct timespec         it_value;       /* current value */
    SYSFUN_TimerState_T     state;
    UI8_T                   valid;
} SYSFUN_TimerEntry_T;
...
#define SYSFUN_MAX_NBR_TASK     100
#define SYSFUN_MAX_NBR_TIMER    100
static SYSFUN_TlsEntry_T    tls_entries[SYSFUN_MAX_NBR_TASK];
static SYSFUN_TimerEntry_T  timer_entries[SYSFUN_MAX_NBR_TIMER];


/* declarations for system call
 */

UI32_T sys_SYSFUN(UI32_T cmd, UI32_T arg1, UI32_T arg2, UI32_T arg3, UI32_T arg4, UI32_T arg5)
{
 ...
 */
static SYSFUN_TlsEntry_T * SYSFUN_GetTlsEntry(int tid);
static SYSFUN_TlsEntry_T * SYSFUN_GetTlsEntryByMsgQId(int qid);
static int SYSFUN_SetTlsEventId();
...
static void SYSFUN_AlarmHandler(int sig_num, siginfo_t *info_p, void *ucontext);
...
{
#if 1
    SYSFUN_TlsEntry_T *tls_p;
    
    tls_p = SYSFUN_GetTlsEntryByMsgQId(msgq_desc_p->request_qid);
    if (tls_p)
    {
        msgq_desc_p->owner_tid = tls_p->tid;
    }
    else
    {
...
void SYSFUN_Init (void)
{
    // init TLS
    {
        memset(&tls_entries, 0, sizeof(tls_entries));
    }
}
...
UI32_T SYSFUN_CreateMsgQ(UI32_T msgq_key, UI32_T msgq_type, UI32_T *msgq_handle_p)
{
    ...
    tls_p = SYSFUN_GetTlsEntry(gettid());
    tls_p->msgq_desc_p = msgq_desc_p;
...
UI32_T SYSFUN_SendEvent (UI32_T tid, UI32_T event)
{
    ...
    SYSFUN_TlsEntry_T *tls_p = SYSFUN_GetTlsEntry(tid);
                            ...
                            UI32_T *received_events)
{
    ...
    SYSFUN_TlsEntry_T *tls_p = SYSFUN_GetTlsEntry(gettid());
...
RECV_AGN:
    ...
    if (tls_p->event & wait_events)
    {
        *received_events = tls_p->event & wait_events;
        tls_p->event &= ~wait_events;
...
static SYSFUN_TimerEntry_T * _SYSFUN_GetFreeTimerEntry()
{
    int i;
    
    for (i=0; i<_countof(timer_entries); ++i)
    {
        SYSFUN_TimerEntry_T *tm_p = &timer_entries[i];
        
        if (FALSE == tm_p->valid)
        {
            memset(tm_p, 0, sizeof(*tm_p));
            
            tm_p->timer_cb.timer_id = i;
            tm_p->timer_cb.pid      = gettid();
            
            tm_p->state             = SYSFUN_TIMER_STOP;
...
static void _SYSFUN_TimerHandler(int signo)
{
    int i;
    
    //printf("tid=%lu signo=%d\r\n", gettid(), signo);

    for (i=0; i<_countof(timer_entries); ++i)
    {
        SYSFUN_TimerEntry_T *tm_p = &timer_entries[i];
        
        if (TRUE == tm_p->valid && SYSFUN_TIMER_RUNNING == tm_p->state)
        {
            if (0 < tm_p->it_value.tv_sec)
            {
                -- (tm_p->it_value.tv_sec);
                
                if (0 == tm_p->it_value.tv_sec)
                {
                    SYSFUN_SendEvent(tm_p->timer_cb.pid, tm_p->timer_cb.event);
                    
                    if (tm_p->timer_cb.callback_func)
                    {
                        tm_p->timer_cb.callback_func();
                    }
                    
                    tm_p->it_value = tm_p->it_interval;
                }
            }
        }
    }
...
static void _SYSFUN_InstallTimerHandler()
{
    static int install_timer_handler = 0;
 
    return;
    
    if (0 == install_timer_handler)
    {
        int ret;
        struct itimerval delay;
        
        signal(SIGALRM, _SYSFUN_TimerHandler);
...
{
#if 1
    SYSFUN_TimerEntry_T *tm_p = _SYSFUN_GetFreeTimerEntry();
...
{
#if 1
    SYSFUN_TimerEntry_T *tm_p = (SYSFUN_TimerEntry_T * ) tm_id;
    struct timespec     ts;
    sigset_t            set;
    ...
    tm_p->timer_cb.event = event;
    
    SYSFUN_TicksToTimespec(time_interval, &ts);
    if (0 == ts.tv_sec)
    {
        sigprocmask(SIG_UNBLOCK, &set, NULL);
        return FALSE;   // tv_sec must largger than 1
    }
    
    tm_p->it_interval   = ts;
    tm_p->it_value      = ts;
    tm_p->state         = SYSFUN_TIMER_RUNNING;
    
    _SYSFUN_InstallTimerHandler();
...
UI32_T  SYSFUN_Sleep (UI32_T  ticks)
{
    ...
    /*
     ...
     The number of clock ticks per second = sysconf(_SC_CLK_TCK));
     
     */
    
    timeout = (UI32_T)times(&buf) + ticks;
...
UI32_T SYSFUN_OpenUART(UI32_T UART_channel)
{
    ...
    // termios.c_lflag &= ~( ECHO | ICANON | ECHOE | ECHOK | ECHONL | ECHOPRT | ECHOKE | ICRNL);
...
static SYSFUN_TlsEntry_T * SYSFUN_GetTlsEntry(int tid)
{
    ...
    for (i=0; i<sizeof(tls_entries)/sizeof(*tls_entries); ++i)
    {
        SYSFUN_TlsEntry_T *tls_p = &tls_entries[i];
...
static SYSFUN_TlsEntry_T * SYSFUN_GetTlsEntryByMsgQId(int qid)
{
    ...
    for (i=0; i<sizeof(tls_entries)/sizeof(*tls_entries); ++i)
    {
        ...
        if (FALSE == tls_p->valid || NULL == tls_p->msgq_desc_p)
        {
            continue;
        }

        if (qid == tls_p->msgq_desc_p->request_qid)
        {
            return tls_p;
        }
    }
...
static int SYSFUN_SetTlsEventId()
{
    static int event_cnt = 1;
    
    int i;
    int tid = gettid();
    SYSFUN_TlsEntry_T *tls_p;
    
    tls_p = SYSFUN_GetTlsEntry(tid);
    if (tls_p && tls_p->event_id != 0)
    {
        return tls_p->event_id;
    }
...
#ifdef HAVE_SYSFUN_UTEST
#include "sysfun_utest.c"
#endif /* HAVE_SYSFUN_UTEST */
