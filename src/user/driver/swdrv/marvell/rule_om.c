 ...
 */
#define RULE_OM_DFLT_METER_RATE                     SYS_ADPT_DIFFSERV_MIN_POLICE_RATE
#define RULE_OM_DFLT_METER_BURST_SIZE               SYS_ADPT_DIFFSERV_MIN_POLICE_BURST
...
#define RULE_OM_IS_TRUNK(ifindex)        ( ifindex >= SYS_ADPT_TRUNK_1_IF_INDEX_NUMBER &&   \
...
static UI32_T
RULE_OM_LocalValidateActionBitmap(
    ...
    UI32_T acl_index)
{
    ...
    /* FIXME: User config and active should use the different
     *        database.
     */
#if (SYS_CPNT_NETACCESS_DYNAMIC_QOS == TRUE)
...
#if (SYS_CPNT_DAI == TRUE)
            if (acl_p->acl_type == RULE_TYPE_ARP_ACL) {
                RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_FAIL);
            }
#endif
...
UI32_T RULE_OM_Add_PolicyMapElement_To_PolicyMap(UI32_T policy_map_index, UI32_T class_map_index)
{
    ...
    /* once policy-map element is created, a default action MUST create together */
...
UI32_T RULE_OM_SetActionEntry(UI32_T action_index, const RULE_TYPE_Action_T *action_entry)
{
    ...
    if (RULE_TYPE_OK != RULE_OM_LocalValidateAction(action_entry))
        RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_FAIL); /* bad parameters */
...
UI32_T RULE_OM_DupActionEntryWithNewActionBitmap(UI32_T action_index, const UI8_T *action_bitmap, RULE_TYPE_Action_T *dst_action_p)
{
...
#if (SYS_CPNT_QOS_V2_SOFTWARE_BUNDLE_RATE_CONTROL == TRUE)
    if (RULE_OM_IS_BIT_ON(action_bitmap, VAL_diffServActionList_actionPktPri))
        core_layer_bitmap |= RULE_TYPE_ACTION_PKT_PRI;
#endif /* SYS_CPNT_QOS_V2_SOFTWARE_BUNDLE_RATE_CONTROL */

    /* check core_layer_bitmap valid or not */
    if (RULE_TYPE_OK != RULE_OM_LocalValidateActionBitmap(core_layer_bitmap))
    {
        LOG("invalid action bitmap(%lu)", core_layer_bitmap);
        RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_FAIL);
    }
    ...
    if (0 == (RULE_TYPE_ACTION_PKT_NEW_DSCP & core_layer_bitmap))
        dst_action_p->pkt_new_dscp = 0;

    if (0 == (RULE_TYPE_ACTION_RED_PKT_NEW_DSCP & core_layer_bitmap))
        dst_action_p->red_pkt_new_dscp = 0;

    if (0 == (RULE_TYPE_ACTION_YELLOW_PKT_NEW_DSCP & core_layer_bitmap))
        dst_action_p->yellow_pkt_new_dscp = 0;
...
UI32_T RULE_OM_BindElement2Acl(UI32_T acl_index, RULE_TYPE_AclType_T ace_type, UI32_T ace_index)
{
    ...
    if (FALSE == RULE_OM_LocalIsAclTypeMatched (acl_type, ace_type))
    {
    ...
    {
        LOG("RULE_OM_LocalValidateAce failed.");
        RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_FAIL);
    }
...
UI32_T RULE_OM_GetUIAclByIndex(UI32_T acl_index, RULE_TYPE_UI_AclEntry_T *acl_entry)
{
    ...
    RULE_OM_LocalConvert2UIAcl(acl_p, acl_entry);
 ...
 * OUTPUT:   acl_index, 
 *           time_range_name  - if time_range_name is NULL,
 *                              it means no need to get time_range_name
 * RETURN:   Error code.
 ...
 * PURPOSE:  Get next binding ACL by port
 * INPUT:    uport_ifindex, acl_index, precedence
 * OUTPUT:   acl_index, precedence, acl_entry, 
 *           time_range_name  - if time_range_name is NULL, 
 *           it means no need to output time_range_name.
 * RETURN:   Error code.
 * NOTE:     none
 *------------------------------------------------------------------------------
 */
...
UI32_T RULE_OM_GetNextUIAclByPortAndBindingType(
    UI32_T precedence, 
    UI32_T *ifindex_p, 
    RULE_TYPE_UI_AclEntry_T *acl_entry_p,
    UI8_T *time_range_name)
{
    RULE_TYPE_RETURN_TYPE_T result;
    ...
    RULE_TYPE_AclType_T ace_type)
{
    ...
    switch (ace_type)
    {
        ...
        case RULE_TYPE_IPV6_STD_ACL:
        {
            ...
            ipv6_ace_p->aceIcmpType     = RULE_TYPE_UNDEF_ICMP_TYPE;
    ...
    RULE_TYPE_COUNTER_ENABLE_T *counter_enable_p)
{
    RULE_TYPE_PortEntry_T       *port_entry_p;
    RULE_TYPE_Acl_T             *acl_p;
    RULE_TYPE_CLASS_INSTANCE_PTR_T acl_instance;
    RULE_TYPE_RETURN_TYPE_T     ret_code;
    UI32_T                      precedence;
    UI32_T                      hardware_acl_index;
    BOOL_T                      get_input_acl;

    ASSERT(NULL != acl_index_p);
    ASSERT(NULL != ace_index_p);
    ASSERT(NULL != ace_entry_p);

    if ((NULL == acl_index_p) || (NULL == ace_index_p) || (NULL == ace_entry_p))
    {
        LOG("Parameter Error");
        return RULE_TYPE_FAIL;
    }

    LOG("Input ifindex: %lu, acl_index: %lu, ace_index: %lu",
        ifindex, *acl_index_p, *ace_index_p);

    precedence = RULE_TYPE_PRECEDENCE_FIRST;
    *counter_enable_p = RULE_TYPE_COUNTER_DISABLE;
    /* TRUE if had input *acl_index */
    get_input_acl = (0 == *acl_index_p) ? FALSE : TRUE;
    ...
    if (0 != *acl_index_p)
    {
        ...
        hardware_acl_index = acl_p->aggregate_acl_index;
    }
    else
    {
        hardware_acl_index = *acl_index_p;
    }
#else
    hardware_acl_index = *acl_index_p;
...
//    acl_instance = RULE_OM_LocalGetNextPortAclInstance(port_entry_p, &precedence);
    acl_instance = NULL;
    if (NULL == acl_instance)
    {
        LOG("NULL GET NEXT ACL");
        return RULE_TYPE_FAIL; /* no next acl */
    }

    LOG("Get next ACL, acl_index: %lu precedence: %lu",
        acl_instance->acl_id, precedence);

    if (TRUE == get_input_acl)
    {
        if (hardware_acl_index == acl_instance->acl_id)
        {
            get_input_acl = FALSE;
        }
        else
    ...
    acl_p = RULE_OM_LocalGetAclByIndex(acl_instance->acl_id);
    if (NULL == acl_p)
    {
        LOG("Get ACL by index fail, acl_index: %lu", acl_instance->acl_id);
        return RULE_TYPE_FAIL;
    }
    ...
    if (TRUE == acl_p->flag_hide)
    {
        switch (acl_p->owner.class_type)
        {
            case RULE_TYPE_CLASS_ACL:

                acl_p = RULE_OM_LocalGetAclByIndex( acl_p->owner.class_id );
                if (NULL == acl_p)
                {
                    LOG("Get ACL by index fail, acl_index: %lu", acl_instance->acl_id);
                    return RULE_TYPE_FAIL;
            ...
            case RULE_TYPE_CLASS_CLASS_MAP:
                /* skip this ACL
                 */
                LOG("Get next ACL");
                goto get_next_acl;
     ...
     */
    {
        ...
        ret_code = RULE_OM_GetNextAceByAcl(acl_instance->acl_id, &ace_type, ace_index_p, ace_entry_p);
        if (RULE_TYPE_OK != ret_code)
        {
            *ace_index_p = 0;
            LOG("Get next ACL");
            goto get_next_acl;
...
#if (SYS_CPNT_ACL_COUNTER == TRUE)
        /* fill counter */
        RULE_CTRL_GetAclRuleCounter(ifindex,
                                    inout_profile,
                                    acl_instance->acl_id,
                                    *ace_index_p,
                                    ace_entry_p);
...
#if (SYS_CPNT_ACL_AUTO_COMPRESS_ACE == TRUE)
    *acl_index_p = port_entry_p->user_config_acl_index[precedence];
#else
    *acl_index_p = acl_instance->acl_id;
#endif /* #if (SYS_CPNT_ACL_AUTO_COMPRESS_ACE == TRUE) */

#if (SYS_CPNT_ACL_COUNTER == TRUE)
    *counter_enable_p = acl_instance->counter_enable;
#endif /* SYS_CPNT_ACL_COUNTER */
 ...
 * INPUT    : ifindex, direction, acl_index, time_range_index, counter
 * OUTPUT   : none
 * RETURN   : none
 * NOTE     : the time_range_index must get from TIME_RANGE_PMGR_RequestTimeRangeEntry
 *------------------------------------------------------------------------------
 */
RULE_TYPE_RETURN_TYPE_T
RULE_OM_SetAcl(
...
/****************** start to compare destination port *********************/
    if (src_ace_p->aceDestPortOp != agg_ace_p->aceDestPortOp)
    {
        return RULE_TYPE_FAIL;
    }
...
/****************** end to compare destination port *********************/
	

/****************** start to compare source port ************************/	
    if (src_ace_p->aceSourcePortOp != agg_ace_p->aceSourcePortOp)
    {
        return RULE_TYPE_FAIL;
    }
    ...
    RULE_TYPE_Ace_Entry_T   *agg_ace_p)
{
    ...
    RULE_TYPE_AclType_T ace_mix_type = RULE_TYPE_MAC_ACL;
    ...
    switch (src_ace_p->ace_type)
    {
        case RULE_TYPE_MAC_ACL:
            ...
            {
                if ((src_ace_p->u.mac.aceEtherTypeOp == VAL_diffServMacAceEtherTypeOp_equal) &&
                    (src_ace_p->u.mac.aceEtherTypeBitmask == MAX_diffServMacAceEtherTypeBitmask))
                {
                    switch (src_ace_p->u.mac.aceMinEtherType) {
                        case RULE_TYPE_ETHERTYPE_IP:
                            ace_mix_type = RULE_TYPE_IP_EXT_ACL;
                            break;
                        case RULE_TYPE_ETHERTYPE_IPV6:
                            ace_mix_type = RULE_TYPE_IPV6_EXT_ACL;
                            break;
                        default:
                            ace_mix_type = RULE_TYPE_MAC_ACL;
                            break;
                    }
                }

                if ((agg_ace_p->u.mac.aceEtherTypeOp == VAL_diffServMacAceEtherTypeOp_equal) &&
                    (agg_ace_p->u.mac.aceEtherTypeBitmask == MAX_diffServMacAceEtherTypeBitmask))
                {
                    switch (agg_ace_p->u.mac.aceMinEtherType) {
                        case RULE_TYPE_ETHERTYPE_IP:
                            if (ace_mix_type == RULE_TYPE_IPV6_EXT_ACL)
                            {
                                return RULE_TYPE_FAIL;
                            }
                            ace_mix_type = RULE_TYPE_IP_EXT_ACL;
                            break;
                        case RULE_TYPE_ETHERTYPE_IPV6:
                            if (ace_mix_type == RULE_TYPE_IP_EXT_ACL)
                            {
                                return RULE_TYPE_FAIL;
                            }
                            ace_mix_type = RULE_TYPE_IPV6_EXT_ACL;
    ...
    RULE_TYPE_UI_BundleGroup_T *ui_bundle_group_p)
{
    ...
    ret = RULE_OM_LocalConvert2UIBundleGroup(
               &policy_map_p->bundle_group[index],
               index + 1,
               ui_bundle_group_p);
    if (RULE_TYPE_OK != ret)
    {
...
static UI32_T RULE_OM_LocalSetAceEntryToDefaultValue(RULE_TYPE_Ace_Entry_T *ace_entry)
{
    ...
    /* need to take mib into consideration,
     *   if need to modify the default values here.
     */
    switch (ace_entry->ace_type)
    {
        ...
        case RULE_TYPE_IP_ACL:
            ...
            ip_ace->aceIcmpType = RULE_TYPE_UNDEF_ICMP_TYPE;
...
set_mac_ace_to_default:
        ...
        case RULE_TYPE_IPV6_EXT_ACL:
        {
            ...
            ipv6_ace->aceSourcePortOp = VAL_diffServIpAceSourcePortOp_noOperator;
            ipv6_ace->aceDestPortOp = VAL_diffServIpAceDestPortOp_noOperator;
            ipv6_ace->aceIcmpType = RULE_TYPE_UNDEF_ICMP_TYPE;
    ...
    UI32_T *ace_index_p)
{
         ...
         RULE_TYPE_OK == RULE_OM_LocalGetNextAceByAcl(acl_p, ace_index_p, &w_ace_entry_p);)
    {
        ...
        case RULE_TYPE_MAC_ACL:
            ...
            {
#if (RULE_TYPE_ACE_MAC_IP_MIX == TRUE)
                if ((w_ace_entry_p->u.mac.aceEtherTypeOp == VAL_diffServMacAceEtherTypeOp_equal) &&
                    (w_ace_entry_p->u.mac.aceEtherTypeBitmask == MAX_diffServMacAceEtherTypeBitmask))
                {
                    switch (w_ace_entry_p->u.mac.aceMinEtherType) {
                        case RULE_TYPE_ETHERTYPE_IP:
                            if (0 == memcmp(&(w_ace_entry_p->u.ip),
                                            &(cmp_entry.u.ip),
                                            sizeof(RULE_TYPE_IpAce_Entry_T)))
    ...
    const RULE_TYPE_Ace_Entry_T *dst_p)
{
    ...
    switch (src_p->ace_type)
    {
        ...
        compare_mac_ace:
            ...
            {
#if (SYS_CPNT_ACL_MAC_IP_MIX == TRUE)
                if ((src_p->u.mac.aceEtherTypeOp == VAL_diffServMacAceEtherTypeOp_equal) &&
                    (src_p->u.mac.aceEtherTypeBitmask == MAX_diffServMacAceEtherTypeBitmask))
                {
                    switch (src_p->u.mac.aceMinEtherType) {
                        case RULE_TYPE_ETHERTYPE_IP:
                            if (0 == memcmp(&src_p->u.ip,
                                            &dst_p->u.ip,
                                            sizeof(dst_p->u.ip)))
                            {
                                return TRUE;
        ...
        case RULE_TYPE_IPV6_STD_ACL:
        case RULE_TYPE_IPV6_EXT_ACL:
            if (0 == memcmp (&src_p->u.ipv6, &dst_p->u.ipv6,
                             sizeof(dst_p->u.ipv6)))
            {
#if (RULE_TYPE_ACE_MAC_IP_MIX == TRUE)
                goto compare_mac_ace;
...
#define FIELD(val)  {#val, val}
        ...
        ace_p->u.mac.aceEtherTypeBitmask == MAX_diffServMacAceEtherTypeBitmask)
    {
        ...
        {
            case RULE_TYPE_ETHERTYPE_IP:
                res = RULE_OM_SetAcePatternToDefaultByType(&dflt_ace_entry,
                                                           RULE_TYPE_IP_ACL);
                ASSERT(res == RULE_TYPE_OK);

                if (0 != memcmp(&ace_p->u.ip, &dflt_ace_entry.u.ip, sizeof(ace_p->u.ip)))
                {
                    struct
                    {
                        char *strval;
                    ...
                    {
                        FIELD(LEAF_diffServMacAceSourceIpAddr),
                        FIELD(LEAF_diffServMacAceSourceIpAddrBitmask),
                        FIELD(LEAF_diffServMacAceDestIpAddr),
                        FIELD(LEAF_diffServMacAceDestIpAddrBitmask),
                        FIELD(LEAF_diffServMacAceIpProtocol),
                        FIELD(LEAF_diffServMacAceL4SourcePortOp),
                        FIELD(LEAF_diffServMacAceL4SourcePort),
                        FIELD(LEAF_diffServMacAceL4SourcePortBitmask),
                        FIELD(LEAF_diffServMacAceL4DestPortOp),
                        FIELD(LEAF_diffServMacAceL4DestPort),
                        FIELD(LEAF_diffServMacAceL4DestPortBitmask),
                    };

                    for (i = 0; i < sizeof(ipv4_fields)/sizeof(*ipv4_fields); ++i)
                    {
                        res = RULE_OM_LocalValidateMacAceMixedFieldForACL(ace_p,
                                                                          ace_p->ace_type,
                                                                          ipv4_fields[i].val);
                        if (RULE_TYPE_OK != res)
                        {
            ...
            case RULE_TYPE_ETHERTYPE_IPV6:
                ...
                if (0 != memcmp(&ace_p->u.ipv6, &dflt_ace_entry.u.ipv6, sizeof(ace_p->u.ipv6)))
                {
                    struct
                    {
                        char *strval;
                    ...
                    {
                        FIELD(LEAF_diffServMacAceSourceIpv6Addr),
                        FIELD(LEAF_diffServMacAceSourceIpv6AddrPrefixLen),
                        FIELD(LEAF_diffServMacAceDestIpv6Addr),
                        FIELD(LEAF_diffServMacAceDestIpv6AddrPrefixLen),
                        FIELD(LEAF_diffServMacAceIpProtocol),
                        FIELD(LEAF_diffServMacAceL4SourcePortOp),
                        FIELD(LEAF_diffServMacAceL4SourcePort),
                        FIELD(LEAF_diffServMacAceL4SourcePortBitmask),
                    ...
                    for (i = 0; i < sizeof(ipv6_fields)/sizeof(*ipv6_fields); ++i)
                    {
                        res = RULE_OM_LocalValidateMacAceMixedFieldForACL(ace_p,
                                                                          ace_p->ace_type,
                                                                          ipv6_fields[i].val);
                        if (RULE_TYPE_OK != res)
                        {
                            LOG("Field %s(%lu) failed", ipv6_fields[i].strval, ipv6_fields[i].val);
                            return res;
                        }
                    }
                }
...
/*------------------------------------------------------------------------------
 * ROUTINE NAME  - RULE_OM_LocalValidateActionBitmap
 *------------------------------------------------------------------------------
 ...
 *------------------------------------------------------------------------------
 */
static UI32_T RULE_OM_LocalValidateActionBitmap(UI32_T action_bitmap)
{
#define RULE_OM_SUPPORTED_ACTION    (RULE_TYPE_ACTION_PKT_NEW_PRI | \
                                    RULE_TYPE_ACTION_PKT_NEW_TOS | \
                                    RULE_TYPE_ACTION_PKT_NEW_DSCP | \
                                    RULE_TYPE_ACTION_RED_DROP | \
                                    RULE_TYPE_ACTION_RED_PKT_NEW_DSCP | \
                                    RULE_TYPE_ACTION_PKT_PRI)
    ...
    action_bitmap &= ~(RULE_OM_SUPPORTED_ACTION);
    if (0 < action_bitmap)
    {
...
static UI32_T RULE_OM_LocalValidateMeter(const RULE_TYPE_TBParamEntry_T *meter_entry)
{
    ...
    {
        case VAL_diffServMeterModel_flow:
        case VAL_diffServMeterModel_trTcmColorBlind:
        case VAL_diffServMeterModel_trTcmColorAware:
        case VAL_diffServMeterModel_srTcmColorBlind:
        case VAL_diffServMeterModel_srTcmColorAware:
            break;

        case VAL_diffServMeterModel_default:
        default:
            return RULE_TYPE_FAIL; /* not support now */
    }

    if ((SYS_ADPT_DIFFSERV_MIN_POLICE_RATE > meter_entry->rate) || (SYS_ADPT_DIFFSERV_MAX_POLICE_RATE < meter_entry->rate))
        return RULE_TYPE_FAIL; /* out of range */

    if ((SYS_ADPT_DIFFSERV_MIN_POLICE_BURST > meter_entry->burst_size) || (SYS_ADPT_DIFFSERV_MAX_POLICE_BURST < meter_entry->burst_size))
        return RULE_TYPE_FAIL; /* out of range */
...
static UI32_T RULE_OM_LocalConvert2UIAction(const RULE_TYPE_Action_T* src, RULE_TYPE_UI_Action_T* dst)
{
    ...
    if (src->action_bitmap & RULE_TYPE_ACTION_RED_DROP)
    {
        dst->out_action_type = OUT_ACTION_DROP;
        dst->out_action_value = 0;
    }
    else if (src->action_bitmap & RULE_TYPE_ACTION_RED_PKT_NEW_DSCP)
    {
        dst->out_action_type = OUT_ACTION_POLICED_DSCP_TRANSMIT;
        dst->out_action_value = src->red_pkt_new_dscp;
    }
    else /* no out action */
    {
        dst->out_action_type = OUT_ACTION_INVALID;
        dst->out_action_value = 0;
    }
...
static UI32_T RULE_OM_LocalConvert2MibAction(const RULE_TYPE_Action_T* src, RULE_TYPE_MIB_Action_T* dst)
{
...
#if (SYS_CPNT_QOS_V2_SOFTWARE_BUNDLE_RATE_CONTROL == TRUE)
    if (RULE_TYPE_ACTION_PKT_PRI & src->action_bitmap)
    {
        RULE_OM_SET_BIT_ON(dst->action_bitmap, VAL_diffServActionList_actionPktPri);
    ...
    dst->pkt_new_ip_prec = src->pkt_new_tos;
...
static UI32_T RULE_OM_LocalConvertToAction(const RULE_TYPE_UI_Action_T *src_action, RULE_TYPE_Action_T *dst_action)
{
    ...
    switch (src_action->out_action_type)
    {
        case OUT_ACTION_DROP:
            dst_action->action_bitmap |= RULE_TYPE_ACTION_RED_DROP;
            break;

        case OUT_ACTION_POLICED_DSCP_TRANSMIT:
            dst_action->action_bitmap |= RULE_TYPE_ACTION_RED_PKT_NEW_DSCP;
            dst_action->red_pkt_new_dscp = src_action->out_action_value;
            break;

        case OUT_ACTION_INVALID: /*no out action*/
            break;
...
static UI32_T RULE_OM_LocalSetInProfileActionToDefault(UI32_T action_index)
{
    ...
    action_p->pkt_new_pri = 0;
    action_p->pkt_new_tos = 0;
    action_p->pkt_new_dscp = 0;
    action_p->pkt_new_phb = 0;
    ...
    RULE_TYPE_Ace_Entry_T *ace_p)
{
    ...
    switch (ethertype)
    {
        ...
        case RULE_TYPE_ETHERTYPE_IPV6:
            ...
            if (RULE_TYPE_ICMP_ICMPTYPE_OP_EQUAL == ui_ace_p->icmp.icmp_type.op)
            {
                ace_p->u.ipv6.aceIcmpType = ui_ace_p->icmp.icmp_type.u.s.data;
            }
...
#define IPV6_QUALIFY 0x04
    ...
    if (ace_qualify & IPV6_QUALIFY)
    {
        ...
        /* icmp-type */
        if (RULE_TYPE_UNDEF_ICMP_TYPE != ace_p->u.ipv6.aceIcmpType)
        {
            ui_ace_p->icmp.icmp_type.op = RULE_TYPE_ICMP_ICMPTYPE_OP_EQUAL;
            ui_ace_p->icmp.icmp_type.u.s.data = ace_p->u.ipv6.aceIcmpType;
            ui_ace_p->icmp.icmp_type.u.s.mask = 0xff;
        }
