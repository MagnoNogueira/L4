    ...
    UI32_T redCntr)
{
    ...
    if (dev_counter_p->type == STAT_TYPE_BYTE_BASED_64BIT)
    {
        ...
        ret = rtk_acl_statByteCnt_set(device_id, dev_rule_p->counter_idx, greenCntr+yellowCntr+redCntr);
        if (ret != RT_ERR_OK)
            return;
    }
    else
    {
        ret = rtk_acl_statPktCnt_set(device_id, dev_rule_p->counter_idx, greenCntr+yellowCntr+redCntr);
        if (ret != RT_ERR_OK)
            return;
    }
}
...
#endif /* SYS_CPNT_QOS_DIFFSERV_POLICE_MODE */
    ...
    RULE_TYPE_COUNTER_ENABLE_T counter_enable)
{
    ...
    result = RULE_CTRL_SetAcl(ifindex, direction, acl_index, TRUE,
        ...
        for (direction = RULE_TYPE_INBOUND; direction <= RULE_TYPE_OUTBOUND; ++direction)
        {
            ...
            for (acl_type_idx = 0; acl_type_idx < _countof(acl_type_ary); ++acl_type_idx)
            {
                ...
                if (RULE_TYPE_OK == result)
                {
                    ...
                    {
                        TIME_RANGE_TYPE_ENTRY_T time_entry;
                        TIME_RANGE_ERROR_TYPE_T t_ret;
                    ...
                    acl_inst_obj.get_element_iterator(&acl_inst_obj, &ace_iter);
                    assert(ace_iter.count(&ace_iter) == acl_entry.ace_count);
...
//                         ++i, rule_iter.next(&rule_iter))
                    for (ace_index = 0, i = 0;
                         RULE_TYPE_OK == RULE_OM_GetNextAceByAcl(acl_index, &ace_type, &ace_index, &ace_entry);
                         ++i, ace_iter.next(&ace_iter))
                    {
                        RULE_TYPE_RULE_INSTANCE_T *rule_inst_ptr;
                        RULE_TYPE_CLASS_INSTANCE_T *ace_inst_ptr;
                        RULE_TYPE_CLASS_INSTANCE_OBJECT_T ace_inst_obj;
                        ...
                        ace_inst_ptr = (RULE_TYPE_CLASS_INSTANCE_PTR_T) ace_iter.get_instance(&ace_iter);
                        RULE_CTRL_IN_Priv_ConstructClassInstanceObject(ace_inst_ptr,
                                                                       &ace_inst_obj);

                        ace_inst_obj.get_element_iterator(&ace_inst_obj, &rule_iter);
                        assert(rule_iter.count(&rule_iter) == 1);
                        ...
                        if (RULE_TYPE_INTERFACE_UPORT == interface.type)
                        {
                            ...
                            assert(DEVRM_PBMP_NOT_NULL(rule_inst_ptr->dev_rule_info.port_bmp));
                            assert(DEVRM_PBMP_MEMBER(rule_inst_ptr->dev_rule_info.port_bmp, _phy_port));
                        }
...
RULE_CTRL_UT_Setup()
{
    ...
    TIME_RANGE_MGR_Initiate_System_Resources();
#endif // SYS_CPNT_TIME_BASED_ACL
...
//    }
    ...
    for (index = 0; index < FP_CONFIG_NumberOfFunctionType(); ++index)
    {
        ...
        for (unit = 1; unit <= SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK; ++unit)
        {
            for (device_id = 0; device_id < DEVRM_PMGR_GetNumberOfChips(); device_id++)
            {
                res_p = RULE_CTRL_GetResourceInfo(unit, device_id, grp_info_p->group_id);
...
RULE_CTRL_UT_Validate_Permit_EAPoL_Packet()
{
    ...
    assert(param.action.pkt_cmd == DEVRM_PKT_CMD_FORWARD);
...
#endif /* SYS_ADPT_MAX_NBR_OF_CHIP_PER_UNIT */
    };
...
{
#if (SYS_CPNT_QOS_DIFFSERV_POLICE_MODE & SYS_CPNT_QOS_DIFFSERV_POLICE_FLOW)
    enum
    {
        MAX_UNIT            = 1,
        MAX_PORT            = 3,
...
#endif /* (SYS_CPNT_QOS_DIFFSERV_POLICE_MODE & SYS_CPNT_QOS_DIFFSERV_POLICE_FLOW) */
    return 0;
}
    ...
    for (map_index = 0; map_index < SYS_ADPT_MAX_NBRS_OF_TCPUDP_ACE; ++map_index)
    {
        ...
        for (unit = 1; unit <= SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK; ++unit)
        {
            for (port = 1; port < SYS_ADPT_TOTAL_PORTS_PER_UNIT_ON_BOARD; ++port)
            {
                ...
                assert(DEVRM_PBMP_MEMBER(rs_p->spr_rs.pbmp, phy_port));
            }
        }
        ...
        for (unit = 1; unit <= SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK; ++unit)
        {
            for (port = 1; port < SYS_ADPT_TOTAL_PORTS_PER_UNIT_ON_BOARD; ++port)
            {
                ...
                {
                    assert(FALSE == DEVRM_PBMP_MEMBER(rs_p->spr_rs.pbmp, phy_port));
                }
            }
        }
...
int
RULE_CTRL_UT_ProtocolVlan()
{
#if (SYS_CPNT_PROTOCOL_VLAN == TRUE) && (SYS_CPNT_PROTOCOL_VLAN_IMPLEMENTED_BY_RULE == TRUE)
    /* 1. Set rule with ethertype + ip and with priority 7
     * 2. Change priority to 5
     * 3. Remove rules
     */
    RULE_TYPE_RETURN_TYPE_T ret;
    BOOL_T enable;
    UI32_T group_index, vlan_id, nbr_of_type_protocol;
    UI32_T unit_idx, port_idx;
    UI8_T priority;
    UI8_T frame_type[SYS_ADPT_1V_MAX_NBR_OF_FRAME_TYPE];
    UI8_T protocol_value[3][SYS_ADPT_1V_MAX_PROTOCOL_VALUE_BUFFER_LENGTH];
    RULE_TYPE_INTERFACE_INFO_T interface;
    ...
    RULE_CTRL_Ifindex_To_Interface(1, &interface);
    unit_idx = interface.uport.unit - 1;
    port_idx = interface.uport.port - 1;
    ...
    group_index = 1;
    vlan_id = 100;
    nbr_of_type_protocol = 1;
    frame_type[0] = VAL_dot1vProtocolTemplateFrameType_ethernet;
    protocol_value[0][0] = RULE_TYPE_ETHERTYPE_IP / 256;
    protocol_value[0][1] = RULE_TYPE_ETHERTYPE_IP % 256;
    priority = 7;

    ret = RULE_CTRL_SetProtocolVlanEntry(enable, &interface, group_index,
                                         vlan_id, nbr_of_type_protocol,
                                         frame_type, protocol_value, priority);
    assert(ret == RULE_TYPE_OK);
    assert(rule_ctrl_shmem_data_p->protocol_vlan[unit_idx][port_idx][0].ethertype == RULE_TYPE_ETHERTYPE_IP);
    assert(rule_ctrl_shmem_data_p->protocol_vlan[unit_idx][port_idx][0].l2_format == VAL_dot1vProtocolTemplateFrameType_ethernet);
    assert(rule_ctrl_shmem_data_p->protocol_vlan[unit_idx][port_idx][0].spr_rs.rule_storage.is_enable == TRUE);
    ...
    ret = RULE_CTRL_SetProtocolVlanEntry(enable, &interface, group_index,
                                         ...
                                         frame_type, protocol_value, priority);
    assert(ret == RULE_TYPE_OK);
    assert(rule_ctrl_shmem_data_p->protocol_vlan[unit_idx][port_idx][0].spr_rs.rule_storage.is_enable == FALSE);
...
int
RULE_CTRL_UT_MacVlanAndProtocolVlan()
{
#if (SYS_CPNT_PROTOCOL_VLAN_IMPLEMENTED_BY_RULE == TRUE) & (SYS_CPNT_MAC_VLAN_IMPLEMENTED_BY_RULE == TRUE)
    /* 1. Add 64 mac-vlan entry
     * 2. Add 1-16 port 4 protocol vlan group entry (64 + 16*4 = 128)
     * 3. Add 17 port protocol vlan group entry shall be failed
     * 4. Remove all
    ...
    ret = RULE_CTRL_GetFunctionInfoByFunctionType(RULE_TYPE_MAC_VLAN, &vlan_group_id, &rule_pri);
    assert(ret == TRUE);

    {
        UI32_T temp_group_id;

        ret = RULE_CTRL_GetFunctionInfoByFunctionType(RULE_TYPE_PROTOCOL_VLAN, &temp_group_id, &rule_pri);
        assert(ret == TRUE);
        assert(temp_group_id == vlan_group_id);
    }
    ...
    {
        UI8_T mac[] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
        UI8_T mask[] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};

        mac[5] = 1+i;
        ret = RULE_CTRL_SetMacVlanEntry(mac, mask, 1, 1);

        assert(ret == TRUE);
        assert(rule_ctrl_shmem_data_p->mac_vlan[i].rule_storage.is_enable == TRUE);
        assert(0 == memcmp(rule_ctrl_shmem_data_p->mac_vlan[i].src_mac_addr, mac, sizeof(mac)));
        assert(0 == memcmp(rule_ctrl_shmem_data_p->mac_vlan[i].src_mac_mask, mask, sizeof(mask)));
    }

    res_p = RULE_CTRL_GetResourceInfo(1, 0, vlan_group_id);
    assert(res_p != NULL);
    assert(res_p->free_rule == res_p->total_rule - SYS_ADPT_MAX_NBR_OF_MAC_VLAN_ENTRY);
    ...
    for (unit = 1;  unit <= SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK; ++unit)
    {
        ...
        {
            for (i = 0; i < SYS_ADPT_1V_MAX_NBR_OF_PROTOCOL_GROUP_ENTRY; i++)
            {
                RULE_TYPE_RETURN_TYPE_T rule_ret;
                RULE_TYPE_INTERFACE_INFO_T interface;
                UI8_T  frame_type = VAL_dot1vProtocolTemplateFrameType_ethernet;
                ...
                rule_ret = RULE_CTRL_SetProtocolVlanEntry(TRUE, &interface, i, 1, 1,
                                                          &frame_type,
                                                          &protocol_value[i], 1);
                assert(rule_ret == RULE_TYPE_OK);
                assert(rule_ctrl_shmem_data_p->protocol_vlan[unit-1][port-1][i].spr_rs.rule_storage.is_enable == TRUE);
                assert(rule_ctrl_shmem_data_p->protocol_vlan[unit-1][port-1][i].l2_format == DEVRM_FIELD_QUALIFY_L2Format_Ethernet);
     ...
     */
    {
        ...
        RULE_TYPE_INTERFACE_INFO_T interface;
        UI8_T  frame_type = VAL_dot1vProtocolTemplateFrameType_ethernet;
        UI8_T  protocol_value[SYS_ADPT_1V_MAX_PROTOCOL_VALUE_BUFFER_LENGTH] = {0x08, 0x00};

        memset(&interface, 0, sizeof(interface));
        interface.type = RULE_TYPE_INTERFACE_UPORT;
        interface.uport.unit = 1;
        interface.uport.port = 17;

        rule_ret = RULE_CTRL_SetProtocolVlanEntry(TRUE, &interface, 0, 1, 1,
                                                  &frame_type,
                                                  &protocol_value, 1);
        assert(rule_ret != RULE_TYPE_OK);
    ...
    for (i = 0; i < SYS_ADPT_MAX_NBR_OF_MAC_VLAN_ENTRY; i++)
    {
        ...
        mac[5] = 1+i;
        ret = RULE_CTRL_DeleteMacVlanEntry(mac, mask);

        assert(ret == TRUE);
        assert(rule_ctrl_shmem_data_p->mac_vlan[i].rule_storage.is_enable == FALSE);
    }
    ...
    for (unit = 1;  unit <= SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK; ++unit)
    {
        for (port = 1; port <= 16; ++port)
        {
            for (i = 0; i < SYS_ADPT_1V_MAX_NBR_OF_PROTOCOL_GROUP_ENTRY; i++)
            {
                ...
                rule_ret = RULE_CTRL_SetProtocolVlanEntry(FALSE, &interface, i, 1, 1,
                                                          &frame_type,
                                                          &protocol_value[i], 1);
                assert(rule_ret == RULE_TYPE_OK);
                assert(rule_ctrl_shmem_data_p->protocol_vlan[unit-1][port-1][i].spr_rs.rule_storage.is_enable == FALSE);
            }
        }
    }
    ...
    RULE_OM_SetDebugFlag(0);
    DEVRM_SetDebugFlags(0);
#endif /* SYS_CPNT_PROTOCOL_VLAN_IMPLEMENTED_BY_RULE & SYS_CPNT_MAC_VLAN_IMPLEMENTED_BY_RULE */

    return 1;
}

int
RULE_CTRL_UT_ModifiedMyMacMyIpQueue()
{
#if (SYS_ADPT_MODIFY_MGMT_QUEUE_BY_RULE == TRUE)
    UI8_T dip[] = {192, 168, 1, 0};
    int i, retry_i;
    BOOL_T ret;
    ...
    /* Add
     */
    for (retry_i = 0; retry_i < 2; ++retry_i)
    {
        for (i = 0; i < SYS_ADPT_MAX_NBR_OF_RIF; ++i)
        {
            dip[3] = i + 1;
            ret = RULE_CTRL_ModifyCpuQueueForMyMacMyIp(TRUE, dip);
            assert(ret == TRUE);

            assert(0 == memcmp(rule_ctrl_shmem_data_p->my_mac_ip[i].dip,
                               dip,
                               sizeof(rule_ctrl_shmem_data_p->my_mac_ip[i].dip)));
            assert(TRUE == rule_ctrl_shmem_data_p->my_mac_ip[i].rule_storage.is_enable);
        }
    ...
    {
        dip[3] = i + 1;
        ret = RULE_CTRL_ModifyCpuQueueForMyMacMyIp(FALSE, dip);
        assert(ret == TRUE);

        assert(FALSE == rule_ctrl_shmem_data_p->my_mac_ip[i].rule_storage.is_enable);
    }
...
RULE_CTRL_UT_DAI()
{
    ...
    rule_info.arp.all_arp_to_cpu = enable;

    ret = RULE_CTRL_TrapPacket2Cpu(enable, RULE_TYPE_PacketType_ARP_REPLY, &rule_info);
    assert(TRUE == ret);

    ret = RULE_CTRL_TrapPacket2Cpu(enable, RULE_TYPE_PacketType_ARP_REQUEST, &rule_info);
    assert(TRUE == ret);
    ...
    for (device_id = 0; device_id < soc_ndev; device_id++)
    {
        ...
        ret = DEVRM_PMGR_CheckSoc(device_id);
        assert(TRUE == ret);

        ret = RULE_CTRL_GetCpuRule(device_id, RULE_TYPE_Pkt2CpuRule_ARP_REPLY, &is_enable, &rule_index);
        assert(TRUE == ret);
        assert(FALSE == is_enable);

        ret = RULE_CTRL_GetCpuRule(device_id, RULE_TYPE_Pkt2CpuRule_ARP_REQUEST, &is_enable, &rule_index);
        assert(TRUE == ret);
        assert(FALSE == is_enable);

        ret = RULE_CTRL_GetCpuRule(device_id, RULE_TYPE_Pkt2CpuRule_ALL_ARP_REPLY, &is_enable, &rule_index);
        assert(TRUE == ret);
        assert(TRUE == is_enable);

        ret = RULE_CTRL_GetCpuRule(device_id, RULE_TYPE_Pkt2CpuRule_ALL_ARP_REQUEST, &is_enable, &rule_index);
        assert(TRUE == ret);
        assert(TRUE == is_enable);
    ...
    for (device_id = 0; device_id < soc_ndev; device_id++)
    {
        ...
        ret = RULE_CTRL_GetCpuRule(device_id, RULE_TYPE_Pkt2CpuRule_ARP_REQUEST, &is_enable, &rule_index);
        assert(TRUE == ret);
        assert(FALSE == is_enable); /* no copy request rule */
    }
#endif /* SYS_CPNT_DAI */
    return 1;
}
...
RULE_CTRL_UT_DAI_WithMeter()
{
    ...
    for (unit = 1; unit <= SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK; ++unit)
    {
        for (port = 1; port <= SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT; ++port)
        {
            ...
            rate = (unit - 1) * SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT + port;
            ret = RULE_CTRL_DAI_TrapArpPacketToCpuWithRate(enable, &interface, rate);
            assert(ret == RULE_TYPE_OK);

            assert(rule_ctrl_shmem_data_p->arp[unit-1][port-1].rule_storage.is_enable == TRUE);
            assert(rule_ctrl_shmem_data_p->arp[unit-1][port-1].meter_rate = rate);
    ...
    for (unit = 1; unit <= SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK; ++unit)
    {
        for (port = 1; port <= SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT; ++port)
        {
            ...
            assert(rule_ctrl_shmem_data_p->arp[unit-1][port-1].rule_storage.is_enable == FALSE);
...
RULE_CTRL_UT_DOS_RateLimit()
{
    ...
    RULE_CTRL_SYS_RS_DOS_RATE_T *rs_p[SWDRV_DOS_RATELIMIT_WIN_NUKE+1];

    rs_p[SWDRV_DOS_RATELIMIT_ECHO_CHARGEN] = &rule_ctrl_shmem_data_p->dos_chargen;
    rs_p[SWDRV_DOS_RATELIMIT_TCP_FLOODING] = &rule_ctrl_shmem_data_p->dos_tcp_flooding;
    rs_p[SWDRV_DOS_RATELIMIT_UDP_FLOODING] = &rule_ctrl_shmem_data_p->dos_udp_flooding;
    rs_p[SWDRV_DOS_RATELIMIT_WIN_NUKE] = &rule_ctrl_shmem_data_p->dos_win_nuk;

    for (i = 0; i < _countof(dos_type); ++i)
    {
        BOOL_T ret;

        rate += i * 100;
        ret = RULE_CTRL_SetDosProtectionRateLimit(dos_type[i], rate);
        assert(ret == TRUE);

        assert(rs_p[dos_type[i]]->rule_storage.is_enable == TRUE);
        assert(rs_p[dos_type[i]]->rate == rate);

        if (dos_type[i] == SWDRV_DOS_RATELIMIT_ECHO_CHARGEN)
        {
            assert(rule_ctrl_shmem_data_p->dos_echo.rule_storage.is_enable == TRUE);
            assert(rule_ctrl_shmem_data_p->dos_echo.rate == rate);
        }
    }
    ...
    for (i = 0; i < _countof(dos_type); ++i)
    {
        ...
        assert(rs_p[dos_type[i]]->rule_storage.is_enable == FALSE);
        assert(rs_p[dos_type[i]]->rate == rate);

        if (dos_type[i] == SWDRV_DOS_RATELIMIT_ECHO_CHARGEN)
        {
            assert(rule_ctrl_shmem_data_p->dos_echo.rule_storage.is_enable == FALSE);
            assert(rule_ctrl_shmem_data_p->dos_echo.rate == rate);
        }
    }
#endif /* SYS_CPNT_DOS */
...
int RULE_CTRL_UT_IpSubnetVlan()
{
    ...
    {
        ret = RULE_CTRL_SetIpSubnetVlanEntry(ip+i, mask+i, vid, priority);
        assert(ret == TRUE);

        assert(rule_ctrl_shmem_data_p->ip_subnet_vlan[i].rule_storage.is_enable == TRUE);
        assert(rule_ctrl_shmem_data_p->ip_subnet_vlan[i].sip_addr.un_w == ip+i);
        assert(rule_ctrl_shmem_data_p->ip_subnet_vlan[i].sip_mask.un_w == mask+i);
    }
    ret = RULE_CTRL_SetIpSubnetVlanEntry(ip + SYS_ADPT_MAX_NBR_OF_IP_SUBNET_VLAN_ENTRY,
                                         mask + SYS_ADPT_MAX_NBR_OF_IP_SUBNET_VLAN_ENTRY,
                                         vid, priority);
    assert(ret == FALSE);

    for (i = 0; i < SYS_ADPT_MAX_NBR_OF_IP_SUBNET_VLAN_ENTRY; ++i)
    {
        ret = RULE_CTRL_DeleteIpSubnetVlanEntry(ip+i, mask+i);
        assert(ret == TRUE);

        assert(rule_ctrl_shmem_data_p->ip_subnet_vlan[i].rule_storage.is_enable == FALSE);
    }
...
//        RULE_CTRL_UT_TEST(RULE_CTRL_UT_Find_Best_Fit_Function_Type);
        ...
        /* Protocol rule
         */
        RULE_CTRL_UT_TEST(RULE_CTRL_UT_ProtocolVlan);
        RULE_CTRL_UT_TEST(RULE_CTRL_UT_MacVlanAndProtocolVlan);
        RULE_CTRL_UT_TEST(RULE_CTRL_UT_ModifiedMyMacMyIpQueue);
        RULE_CTRL_UT_TEST(RULE_CTRL_UT_DAI);
